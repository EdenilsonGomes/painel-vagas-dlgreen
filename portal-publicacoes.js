...
function registerPortalPublications({ app, pool, requireAdmin, currentUserName, portalBaseUrl = '' }) {
    ...
    try {
      ...
      const result = await pool.query(`
        UPDATE gg_groups SET
          status=$1,verified=$2,featured=$3,official=$4,
          rejection_reason=NULLIF($5,''),moderation_note=NULLIF($6,''),
          approved_at=CASE WHEN $1='approved' THEN COALESCE(approved_at,NOW()) ELSE approved_at END,
          last_verified_at=CASE WHEN $2 IS TRUE THEN NOW() ELSE last_verified_at END,
          updated_at=NOW()
        WHERE id=$7 RETURNING *
      `, [parsed.data.status, parsed.data.verified, parsed.data.featured, parsed.data.official, parsed.data.rejection_reason, parsed.data.moderation_note, Number(req.params.id)]);
  ...
  app.patch('/api/portal-publicacoes/vagas/:id', requireAdmin, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`UPDATE portal_vagas_submissoes SET status=$1,rejection_reason=NULLIF($2,''),moderation_note=NULLIF($3,''),approved_at=CASE WHEN $1='APROVADA' THEN COALESCE(approved_at,NOW()) ELSE approved_at END,updated_at=NOW() WHERE id=$4 AND status<>'CONVERTIDA' RETURNING *`, [parsed.data.status, parsed.data.rejection_reason, parsed.data.moderation_note, Number(req.params.id)]);
