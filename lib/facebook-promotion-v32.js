...
function registerFacebookPromotionV32({ app, pool, requireLogin, requireAdmin, currentUserName, publicBaseUrl, portalBaseUrl }) {
  ...
  app.get('/api/admin/divulgacao-facebook/campanhas/:id', adminAccess, async (req, res, next) => {
    try {
      ...
      res.json({ sucesso: true, campanha: campaign, imagem_url: campaign.usar_imagem ? `/api/vagas/${campaign.vaga_id}/divulgacao/principal.png` : null,
  ...
  app.patch('/api/admin/divulgacao-facebook/destinos/:id', adminAccess, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`UPDATE divulgacao_campanha_destinos d SET status=$2::VARCHAR(30),
        publicado_at=CASE WHEN $2::VARCHAR(30)='PUBLICADO' THEN COALESCE(publicado_at,NOW()) ELSE publicado_at END,
        enviado_at=CASE WHEN $2::VARCHAR(30)='ENVIADO' THEN COALESCE(enviado_at,NOW()) ELSE enviado_at END,
        erro=CASE WHEN $2::VARCHAR(30)='FALHA' THEN NULLIF($3::TEXT,'') ELSE NULL END,updated_at=NOW()
