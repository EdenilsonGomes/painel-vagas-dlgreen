...
function registerDivulgacaoV1({
  ...
  app.get('/api/divulgacao/bootstrap', async (req, res, next) => {
    try {
      ...
      const [jobs, groups, campaigns, clicks] = await Promise.all([
        pool.query(`SELECT v.id,v.codigo,v.titulo,v.cargo,v.bairro,v.cidade,v.estado,v.salario,v.escala,v.horario,v.beneficios,v.status,e.nome AS empresa_nome FROM vagas v JOIN empresas e ON e.id=v.empresa_id WHERE v.status='ATIVA' ${!isAdmin(req) && req.user?.empresa_id ? 'AND v.empresa_id=$1' : ''} ORDER BY v.updated_at DESC LIMIT 200`, !isAdmin(req) && req.user?.empresa_id ? [req.user.empresa_id] : []),
  ...
  app.post('/api/divulgacao/grupos/importar-facebook', async (req,res,next)=>{
    try{
      const lines=clean(req.body?.conteudo,50000).split(/\r?\n/).map(x=>x.trim()).filter(Boolean).slice(0,500); let imported=0,ignored=0; const errors=[];
      for(const [index,line] of lines.entries()){
        const [nome,url,regiao='',categorias='',regras='']=line.split('|').map(x=>x.trim()); const norm=normalizeFacebookUrl(url);
        if(!nome||!norm){ignored++;errors.push(`Linha ${index+1}: nome ou URL inválidos.`);continue;}
        const result=await pool.query(`INSERT INTO divulgacao_grupos(canal,nome,url,url_normalizada,empresa_id,owner_user_id,regiao,categorias,regras,origem,ativo) VALUES('FACEBOOK',$1,$2,$3,$4,$5,NULLIF($6,''),$7::TEXT[],NULLIF($8,''),'IMPORTACAO',TRUE) ON CONFLICT DO NOTHING RETURNING id`,[nome.slice(0,220),url.slice(0,2000),norm,req.user.empresa_id||null,req.user.id||null,regiao.slice(0,160),splitList(categorias),regras.slice(0,5000)]);
        if(result.rowCount) imported++; else ignored++;
      }
      res.json({sucesso:true,importados:imported,ignorados:ignored,erros:errors.slice(0,20)});
    }catch(error){next(error);}
  });
  ...
  app.get('/api/divulgacao/campanhas', async(req,res,next)=>{
    try{const sc=campaignScope(req,'c');const result=await pool.query(`SELECT c.*,v.titulo AS vaga_titulo,v.cidade,v.bairro,e.nome AS empresa_nome,(SELECT COUNT(*)::INTEGER FROM divulgacao_campanha_destinos d WHERE d.campanha_id=c.id) AS destinos_total,(SELECT COUNT(*)::INTEGER FROM divulgacao_campanha_destinos d WHERE d.campanha_id=c.id AND d.status IN ('ENVIADO','PUBLICADO')) AS concluidos,(SELECT COUNT(*)::INTEGER FROM divulgacao_cliques x JOIN divulgacao_campanha_destinos d ON d.id=x.destino_id WHERE d.campanha_id=c.id) AS cliques FROM divulgacao_campanhas c JOIN vagas v ON v.id=c.vaga_id JOIN empresas e ON e.id=v.empresa_id WHERE ${sc.sql} ORDER BY c.created_at DESC LIMIT 300`,sc.values);res.json({sucesso:true,campanhas:result.rows});}catch(error){next(error);}
  });
  ...
  app.post('/api/divulgacao/campanhas',async(req,res,next)=>{
    const client=await pool.connect();try{
      ...
      const vaga=(await client.query(`SELECT v.*,e.nome AS empresa_nome FROM vagas v JOIN empresas e ON e.id=v.empresa_id WHERE v.id=$1 ${!isAdmin(req)&&req.user?.empresa_id?'AND v.empresa_id=$2':''} LIMIT 1`,!isAdmin(req)&&req.user?.empresa_id?[data.vaga_id,req.user.empresa_id]:[data.vaga_id])).rows[0];if(!vaga)return res.status(404).json({sucesso:false,erro:'Vaga não encontrada ou fora do seu acesso.'});
  ...
  app.get('/api/divulgacao/campanhas/:id',async(req,res,next)=>{
    try{const id=parseId(req.params.id);const sc=campaignScope(req,'c');const result=await pool.query(`SELECT c.*,v.titulo AS vaga_titulo,v.cargo,v.bairro,v.cidade,v.estado,v.salario,v.escala,v.horario,v.beneficios,e.nome AS empresa_nome FROM divulgacao_campanhas c JOIN vagas v ON v.id=c.vaga_id JOIN empresas e ON e.id=v.empresa_id WHERE c.id=$${sc.values.length+1} AND ${sc.sql}`,[...sc.values,id]);if(!result.rowCount)return res.status(404).json({sucesso:false,erro:'Campanha não encontrada.'});const destinations=await pool.query(`SELECT d.*,g.nome AS grupo_nome,g.canal,g.url,g.external_id,g.regiao,g.regras,g.autorizado_envio,g.ultima_publicacao_at,(SELECT COUNT(*)::INTEGER FROM divulgacao_cliques x WHERE x.destino_id=d.id) AS cliques FROM divulgacao_campanha_destinos d JOIN divulgacao_grupos g ON g.id=d.grupo_id WHERE d.campanha_id=$1 ORDER BY d.id`,[id]);const campaign=result.rows[0];const rows=destinations.rows.map(d=>({...d,link_rastreavel:`${normalizedPortalBase}/r/div/${d.tracking_token}`,texto:(d.texto_override||campaign.texto_modelo).replaceAll('{{link}}',`${normalizedPortalBase}/r/div/${d.tracking_token}`)}));res.json({sucesso:true,campanha,destinos:rows});}catch(error){next(error);}
  });

  app.get('/api/divulgacao/vagas/:id/imagem.jpg',async(req,res,next)=>{
    try{const id=parseId(req.params.id);const vacancy=await loadVacancyForPromotion?.(id);if(!vacancy)return res.status(404).json({sucesso:false,erro:'Vaga não encontrada.'});const png=await promotionPng(vacancy);const jpeg=await sharp(png).flatten({background:'#ffffff'}).jpeg({quality:90,mozjpeg:true}).toBuffer();res.setHeader('Content-Type','image/jpeg');res.setHeader('Cache-Control','private, max-age=300');res.send(jpeg);}catch(error){next(error);}
  });
  ...
  app.patch('/api/divulgacao/destinos/:id/status',async(req,res,next)=>{
    try{const id=parseId(req.params.id);const row=await getDestination(req,id);if(!row)return res.status(404).json({sucesso:false,erro:'Destino não encontrado.'});const requested=clean(req.body?.status,30).toUpperCase();const allowed=row.canal==='FACEBOOK'?['PUBLICADO','PULADO','FALHA','PENDENTE']:['PULADO','PENDENTE'];if(!allowed.includes(requested))return res.status(400).json({sucesso:false,erro:'Status não permitido para este canal.'});if(requested==='PUBLICADO'&&row.campanha_status!=='EM_EXECUCAO')return res.status(409).json({sucesso:false,erro:'Inicie a campanha antes de registrar a publicação.'});await pool.query(`UPDATE divulgacao_campanha_destinos SET status=$2,publicado_at=CASE WHEN $2='PUBLICADO' THEN NOW() ELSE publicado_at END,erro=NULLIF($3,''),updated_at=NOW() WHERE id=$1`,[id,requested,clean(req.body?.erro,1500)]);if(requested==='PUBLICADO')await pool.query('UPDATE divulgacao_grupos SET ultima_publicacao_at=NOW(),updated_at=NOW() WHERE id=$1',[row.grupo_id]);res.json({sucesso:true});}catch(error){next(error);}
  });
...
module.exports={registerDivulgacaoV1,normalizeFacebookUrl,normalizeWahaGroups,defaultText};
