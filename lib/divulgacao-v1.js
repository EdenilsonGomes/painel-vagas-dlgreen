...
function registerDivulgacaoV1({
  ...
  app.get('/api/divulgacao/campanhas/:id',async(req,res,next)=>{
    try{
      ...
      res.json({sucesso:true,campanha:campaign,destinos:rows});
    }catch(error){
  ...
  app.patch('/api/divulgacao/destinos/:id/status',async(req,res,next)=>{
    ...
    try{
      if(!id)return res.status(400).json({sucesso:false,erro:'Destino inválido.'});
      const row=await getDestination(req,id);
      if(!row)return res.status(404).json({sucesso:false,erro:'Destino não encontrado.'});
      ...
      // Atualizações por estado, com valores literais. Evita inferência ambígua de tipos
      // do PostgreSQL quando o mesmo parâmetro é usado em status + CASEs de timestamp.
      if(requested==='PUBLICADO'){
        await pool.query(`UPDATE divulgacao_campanha_destinos SET status='PUBLICADO',enviado_at=COALESCE(enviado_at,NOW()),publicado_at=NOW(),erro=NULL,updated_at=NOW() WHERE id=$1`,[id]);
        await pool.query(`UPDATE divulgacao_grupos SET ultima_publicacao_at=NOW(),updated_at=NOW() WHERE id=$1`,[row.grupo_id]);
      }else if(requested==='ENVIADO'){
        await pool.query(`UPDATE divulgacao_campanha_destinos SET status='ENVIADO',enviado_at=COALESCE(enviado_at,NOW()),publicado_at=NULL,erro=NULL,updated_at=NOW() WHERE id=$1`,[id]);
      }else if(requested==='FALHA'){
        await pool.query(`UPDATE divulgacao_campanha_destinos SET status='FALHA',publicado_at=NULL,erro=NULLIF($2::text,''),updated_at=NOW() WHERE id=$1`,[id,reason]);
      }else if(requested==='PULADO'){
        await pool.query(`UPDATE divulgacao_campanha_destinos SET status='PULADO',erro=NULL,updated_at=NOW() WHERE id=$1`,[id]);
      }else if(requested==='PENDENTE'){
        await pool.query(`UPDATE divulgacao_campanha_destinos SET status='PENDENTE',enviado_at=NULL,publicado_at=NULL,erro=NULL,updated_at=NOW() WHERE id=$1`,[id]);
      }

      return res.json({sucesso:true,status:requested});
    }catch(error){
      console.error('[DIVULGAÇÃO V2] Falha ao atualizar status do destino:',{
        id, status:requested, user:req.user?.id, message:error.message, code:error.code, constraint:error.constraint
      });
      next(error);
    }
  });
