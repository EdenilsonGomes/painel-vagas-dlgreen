...
async function fetchBrasilApiCep(cep, { fetchImpl = fetch, timeoutMs = 7000 } = {}) {
  ...
  try {
    ...
    const latitude = parseCoordinate(body?.location?.coordinates?.latitude, 'lat');
    const longitude = parseCoordinate(body?.location?.coordinates?.longitude, 'lon');
    return {
      ok: latitude !== null && longitude !== null,
      status: latitude !== null && longitude !== null ? 'OK' : 'SEM_COORDENADAS',
      data: {
        cep,
      ...
      error: latitude !== null && longitude !== null ? null : 'CEP localizado, mas sem coordenadas disponíveis.',
    };
  } catch (error) {
...
async function resolveCep(pool, rawCep, options = {}) {
  ...
  if (cached?.status === 'OK') return { source: 'CACHE', cep: cached };
  const retryAfter = cached?.status === 'ERRO' ? TRANSIENT_RETRY_AFTER_MS : RETRY_AFTER_MS;
  if (!options.force && cached?.consultado_at && Date.now() - new Date(cached.consultado_at).getTime() < retryAfter) {
    ...
    try {
      const result = await pool.query(`
        SELECT c.id AS candidato_id,c.vaga_id,c.cep AS candidato_cep,gv.cep AS vaga_cep,
          ...
          genesis_geo_distancia_km(gc.latitude,gc.longitude,gvgeo.latitude,gvgeo.longitude) AS distancia_km,
          CASE
            WHEN c.cep IS NULL OR BTRIM(c.cep)='' THEN 'SEM_CEP_CANDIDATO'
            WHEN c.vaga_id IS NULL THEN 'SEM_VAGA'
            WHEN gv.cep IS NULL THEN 'SEM_CEP_VAGA'
            WHEN gc.status='OK' AND gvgeo.status='OK' THEN 'OK'
            ELSE 'PENDENTE'
          END AS geo_status
        FROM candidatos c
  ...
  app.get('/api/geo/status', async (_req, res, next) => {
    try {
      const [cache, candidates, vacancies] = await Promise.all([
        pool.query(`SELECT COUNT(*)::INTEGER AS total,COUNT(*) FILTER(WHERE status='OK')::INTEGER AS ok,COUNT(*) FILTER(WHERE status<>'OK')::INTEGER AS pendentes FROM geo_ceps`),
...
module.exports = { registerGeoV1, normalizeCep, fetchBrasilApiCep, resolveCep, enrichMissingCeps };
