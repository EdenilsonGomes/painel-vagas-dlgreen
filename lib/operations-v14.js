...
function registerOperationsV14({
  ...
  const imageHourlyLimit = Math.min(Math.max(Number(process.env.OPENAI_IMAGE_HOURLY_LIMIT || 4), 1), 20);
  ...
  async function generateVacancyPhoto(vacancy) {
    ...
    const prompt = [
      ...
      'Do not include any words, letters, numbers, logos, watermarks, badges, posters, signs, uniforms with brands, or UI elements.',
    ].join(' ');
    try {
  ...
  app.post('/api/vagas/:id/divulgacao/gerar-ia', requireLogin, async (req, res, next) => {
    ...
    try {
      ...
      const result = await pool.query(`
        SELECT v.*,e.nome AS empresa_nome,m.estilo_visual,m.cor_primaria,m.cor_secundaria,m.cor_destaque
      ...
      const recent = await pool.query('SELECT COUNT(*)::INTEGER AS total FROM vaga_artes_ia WHERE vaga_id=$1 AND created_at>=NOW()-INTERVAL \'1 hour\'', [id]);
      if (Number(recent.rows[0]?.total || 0) >= imageHourlyLimit) return res.status(429).json({ sucesso: false, erro: `Limite de ${imageHourlyLimit} gerações por vaga a cada hora atingido.` });
      const generated = await generateVacancyPhoto(result.rows[0]);
      ...
      res.status(201).json({ sucesso: true, mensagem: 'Nova fotografia criada. Logo e textos foram aplicados separadamente para preservar a marca.', arte: saved.rows[0], imagem_png_url: `/api/vagas/${id}/divulgacao/principal.png?arte=${saved.rows[0].id}&v=${Date.now()}` });
    } catch (error) {
  ...
  app.get('/api/admin/prospeccao/contato/config', requireLogin, requireAdmin, async (req, res, next) => {
    try {
      let session = null;
  ...
  app.post('/api/admin/prospeccao/contato/session/start', requireLogin, requireAdmin, async (_req, res, next) => {
    try {
  ...
  app.get('/api/admin/prospeccao/contato/session/qr', requireLogin, requireAdmin, async (_req, res, next) => {
    try {
  ...
  app.post('/api/admin/prospeccao/leads/:id/agendar-contato', requireLogin, requireAdmin, async (req, res, next) => {
    try {
      const id = parseId(req.params.id);
  ...
  app.post('/api/public/prospeccao/waha', async (req, res, next) => {
    try {
  ...
  return {
    ...
    config: { imageModel, imageQuality, openAiConfigured: Boolean(openAiApiKey), prospectingSession, outreachEnabled },
  };
}
