...
function registerAtendimentoV15({
  ...
  async function renderPublicInterviewAction(req, res, next) {
    try {
      ...
      if (!d) return res.status(404).send('<!doctype html><html lang="pt-BR"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Link indisponível | Gênesis</title><style>body{font-family:Inter,system-ui,Arial,sans-serif;background:#f5fafb;color:#0b1f2a;margin:0;min-height:100vh;display:grid;place-items:center}.box{width:min(calc(100% - 32px),520px);padding:28px;border:1px solid #dce8ea;border-radius:18px;background:#fff;box-shadow:0 18px 48px #0b1f2a14}.brand{display:flex;align-items:center;gap:10px;margin-bottom:26px}.brand img{width:44px;height:44px;object-fit:contain}.brand strong{letter-spacing:.07em}.box p{color:#64748b;line-height:1.6}</style><body><main class="box"><div class="brand"><img src="/assets/brand/genesis-symbol.webp?v=2200" alt=""><strong>GÊNESIS</strong></div><h1>Link indisponível</h1><p>Este link expirou ou não existe. Solicite uma nova opção de horário à equipe responsável pela vaga.</p></main></body></html>');
      ...
      return res.type('html').send(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#0B1F2A"><title>Gerenciar entrevista | Gênesis</title><style>:root{--navy:#0b1f2a;--navy2:#0d2f3a;--teal:#0eaea0;--cyan:#22d3ee;--line:#dce8ea;--muted:#64748b}*{box-sizing:border-box}body{font-family:Inter,system-ui,Arial,sans-serif;background:linear-gradient(180deg,#e6f7fa,#f5fafb 32%);margin:0;color:var(--navy)}.box{max-width:660px;margin:36px auto;background:#fff;border:1px solid var(--line);border-radius:20px;padding:28px;box-shadow:0 18px 48px #0b1f2a16}.brand{display:flex;align-items:center;gap:11px;margin-bottom:26px}.brand img{width:46px;height:46px;object-fit:contain}.brand strong{letter-spacing:.07em}.brand small{display:block;color:#078f85;margin-top:2px}h1{margin:0 0 8px}.meta,.notice{background:#f7fafb;border:1px solid #e4edef;border-radius:12px;padding:16px;margin:20px 0}.actions{display:grid;gap:12px}.btn{display:block;width:100%;border:0;border-radius:10px;padding:14px;font-weight:750;cursor:pointer}.primary{background:linear-gradient(135deg,#078f85,var(--teal));color:#fff}.ghost{background:#f1f5f9;color:var(--navy)}label{display:grid;gap:6px;margin:10px 0}textarea{font:inherit;padding:12px;border:1px solid #c8d9dc;border-radius:10px}.slots{display:grid;gap:10px;margin:14px 0}.slot{display:flex;align-items:center;gap:12px;padding:13px;border:1px solid var(--line);border-radius:10px;cursor:pointer}.slot:has(input:checked){border-color:var(--teal);background:#e4f8f6}.slot input{accent-color:var(--teal)}.slot span{display:grid;gap:2px}.slot small,small{color:var(--muted)}.warn{background:#fff5e6;color:#9a4f08}.notice span{display:block;margin-top:5px}@media(max-width:700px){body{background:#fff}.box{margin:0;min-height:100vh;border-radius:0;border:0;padding:22px}}</style></head><body><main class="box"><div class="brand"><img src="/assets/brand/genesis-symbol.webp?v=2200" alt=""><div><strong>GÊNESIS</strong><small>Pessoas · Oportunidades · Tecnologia</small></div></div><small>ENTREVISTA</small><h1>${escaped(d.candidato_nome)}</h1><p>${escaped(d.vaga_nome)}</p><div class="meta"><strong>Horário atual</strong><p>${escaped(formatPt(d.inicio))}</p></div><div class="actions"><form method="post" action="/api/public/entrevistas/acao/${encodeURIComponent(token)}/confirmar"><button class="btn primary" type="submit">Confirmar horário</button></form><details><summary class="btn ghost">Reagendar entrevista</summary><form method="post" action="/api/public/entrevistas/acao/${encodeURIComponent(token)}/reagendar">${optionsSection}</form></details></div></main></body></html>`);
  ...
  app.post('/api/atendimento/candidatos/:id/mensagens', requireLogin, async (req,res,next)=>{
    ...
    try{
      ...
      await client.query('COMMIT');
      try{
        const response=await triggerManualCandidateMessage({candidatoId:id,mensagem:finalMessage,evento:'MENSAGEM_ATENDIMENTO_HUMANO',solicitadoPor:author,session});
        if(!response?.configurado)throw new AtendimentoV15Error(response?.aviso||'Webhook de atendimento manual não configurado.',503);
        if(response?.enviado===false)throw new AtendimentoV15Error(response?.aviso||'O workflow não confirmou o envio da mensagem.',502);
        const messageId=String(response?.mensagem_id||'').trim();
        const updated=await pool.query(`UPDATE mensagens SET status_envio='ENVIADA',mensagem_id=COALESCE(NULLIF($2,''),mensagem_id) WHERE id=$1 RETURNING *`,[inserted.rows[0].id,messageId]);
