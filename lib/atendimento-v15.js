...
function registerAtendimentoV15({
  ...
  const panelUrl = String(panelBaseUrl || '').replace(/\/$/, '');
  ...
  async function processOperationalNotification() {
    ...
    try {
      ...
      const text = String(item.mensagem || '').replaceAll('{{PANEL_URL}}', panelUrl);
      try {
  ...
  app.get('/entrevistas/acao/:token', async (req, res, next) => {
    try {
      ...
      `, [token]);
      if (!result.rowCount || new Date(result.rows[0].expira_em) < new Date()) return res.status(404).send('<!doctype html><meta charset="utf-8"><title>Link indisponível</title><p>Este link expirou ou não existe.</p>');
      const d = result.rows[0];
      const escaped = (value) => String(value || '').replace(/[&<>"']/g, (char) => ({ '&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;' }[char]));
      return res.type('html').send(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Gerenciar entrevista</title><style>body{font-family:system-ui;background:#f4f7fb;margin:0;color:#152033}.box{max-width:620px;margin:48px auto;background:#fff;border:1px solid #dde5ef;border-radius:20px;padding:28px;box-shadow:0 18px 60px #17203314}h1{margin:0 0 8px}.meta{background:#f7fafc;border-radius:14px;padding:16px;margin:20px 0}.actions{display:grid;gap:12px}.btn{display:block;width:100%;border:0;border-radius:12px;padding:14px;font-weight:700;cursor:pointer}.primary{background:#0f766e;color:#fff}.ghost{background:#e8eef5;color:#152033}label{display:grid;gap:6px;margin:10px 0}input,textarea{font:inherit;padding:12px;border:1px solid #ccd6e2;border-radius:10px}small{color:#607086}</style></head><body><main class="box"><small>GÊNESIS IA · ENTREVISTA</small><h1>${escaped(d.candidato_nome)}</h1><p>${escaped(d.vaga_nome)}</p><div class="meta"><strong>Horário atual</strong><p>${escaped(formatPt(d.inicio))}</p></div><div class="actions"><form method="post" action="/api/public/entrevistas/acao/${encodeURIComponent(token)}/confirmar"><button class="btn primary" type="submit">Confirmar horário</button></form><details><summary class="btn ghost">Solicitar reagendamento</summary><form method="post" action="/api/public/entrevistas/acao/${encodeURIComponent(token)}/reagendar"><label>Novo início<input name="inicio" type="datetime-local" required></label><label>Duração em minutos<input name="duracao" type="number" min="10" max="180" value="30" required></label><label>Motivo<textarea name="motivo" maxlength="500"></textarea></label><button class="btn primary" type="submit">Enviar proposta ao candidato</button></form></details></div></main></body></html>`);
  ...
  app.post('/api/public/entrevistas/acao/:token/reagendar', async (req, res, next) => {
    try {
      const token = clean(req.params.token, 96);
      const start = safeDate(req.body?.inicio);
      const duration = Math.min(180, Math.max(10, Number(req.body?.duracao || 30)));
      const end = new Date(start.getTime() + duration * 60000);
      const lookup = await pool.query(`SELECT e.id entrevista_id FROM entrevista_acao_tokens t JOIN entrevistas e ON e.id=t.entrevista_id WHERE t.token=$1 AND t.expira_em>NOW() LIMIT 1`, [token]);
      if (!lookup.rowCount) throw new AtendimentoV15Error('Link inválido ou expirado.', 404);
      await callInterviewManagement({ action: 'PROPOR_REAGENDAMENTO', entrevista_id: lookup.rows[0].entrevista_id, inicio_proposto: start.toISOString(), fim_proposto: end.toISOString(), motivo: clean(req.body?.motivo, 500), solicitado_por: 'Recrutador pelo WhatsApp' });
  ...
  app.get('/api/atendimento/candidatos/:id/conversa', requireLogin, async (req, res, next) => {
    try {
      ...
      return res.json({ sucesso: true, candidato: { id:candidate.id, ia_atendimento_ativo:candidate.ia_atendimento_ativo, atendimento_humano_ativo:candidate.atendimento_humano_ativo, atendimento_humano_usuario_id:candidate.atendimento_humano_usuario_id, atendimento_responsavel_nome:candidate.atendimento_responsavel_nome, etapa:candidate.etapa, status:candidate.status, proxima_acao:buildManualContinuationMessage(candidate,'') }, mensagens: messages.rows });
    } catch (error) { next(error); }
  });
  ...
  app.post('/api/atendimento/candidatos/:id/devolver', requireLogin, async (req, res, next) => {
    const client=await pool.connect();
    try{
      const id=parseId(req.params.id);if(!id)throw new AtendimentoV15Error('Candidato inválido.');
      await client.query('BEGIN');const candidate=await candidateRow(id,true,client);
      if(candidate.atendimento_humano_ativo&&Number(candidate.atendimento_humano_usuario_id)!==Number(req.user.id)&&!isAdmin(req))throw new AtendimentoV15Error('Somente o responsável atual ou um administrador pode devolver a conversa para a IA.',403);
      const name=currentUserName(req);
      await client.query(`UPDATE candidatos SET ia_atendimento_ativo=TRUE,ia_retomada_em=NOW(),ia_retomada_por=$2,ia_ultima_acao_manual='DEVOLVIDO_PARA_IA',ia_ultima_acao_manual_em=NOW(),ia_ultima_acao_manual_por=$2,atendimento_humano_ativo=FALSE,atendimento_humano_usuario_id=NULL,atendimento_humano_nome=NULL,atendimento_humano_finalizado_em=NOW(),updated_at=NOW() WHERE id=$1`,[id,name]);
      await client.query(`INSERT INTO eventos(candidato_id,evento,descricao,created_at) VALUES($1,'ATENDIMENTO_DEVOLVIDO_PARA_IA',$2,NOW())`,[id,`Atendimento devolvido para a IA por ${name}. Etapa preservada: ${candidate.etapa}.`]);
      await client.query('COMMIT');res.json({sucesso:true,mensagem:'Conversa devolvida para a IA na etapa atual.',proxima_acao:buildManualContinuationMessage(candidate,'')});
    }catch(error){try{await client.query('ROLLBACK');}catch{}next(error);}finally{client.release();}
  });
  ...
  app.post('/api/atendimento/candidatos/:id/mensagens', requireLogin, async (req,res,next)=>{
    ...
    try{
      ...
      }catch(sendError){await pool.query(`UPDATE mensagens SET status_envio='FALHA',contexto_snapshot=COALESCE(contexto_snapshot,'{}'::jsonb)||JSONB_BUILD_OBJECT('erro_envio',$2) WHERE id=$1`,[inserted.rows[0].id,sendError.message]);throw sendError;}
    }catch(error){try{await client.query('ROLLBACK');}catch{}next(error);}finally{client.release();}
  });
  ...
  app.get('/api/atendimento/candidatos/:id/correcao/preview', requireAdmin, async(req,res,next)=>{
  ...
  app.post('/api/atendimento/candidatos/:id/correcao', requireAdmin, async(req,res,next)=>{
    const client=await pool.connect();
    try{
  ...
  app.post('/api/atendimento/entrevistas/:id/reagendar', requireLogin, async(req,res,next)=>{
    try{const id=parseId(req.params.id);if(!id)throw new AtendimentoV15Error('Entrevista inválida.');const start=safeDate(req.body?.inicio);const duration=Math.min(180,Math.max(10,Number(req.body?.duracao||30)));const end=new Date(start.getTime()+duration*60000);const result=await callInterviewManagement({action:'PROPOR_REAGENDAMENTO',entrevista_id:id,inicio_proposto:start.toISOString(),fim_proposto:end.toISOString(),motivo:clean(req.body?.motivo,500),solicitado_por:currentUserName(req),solicitado_por_usuario_id:req.user.id||null});res.json({sucesso:true,mensagem:result.mensagem||'Proposta enviada. O horário atual permanece reservado.',resultado:result});}catch(error){next(error);}
  });
}
