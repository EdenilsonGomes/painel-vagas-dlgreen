...
function registerAtendimentosV16({
    ...
    try {
      ...
      const result = await pool.query(`
        SELECT c.id,c.nome,c.telefone,c.status,c.etapa,c.vaga_id,COALESCE(v.titulo,c.vaga,'Vaga') vaga_nome,
          ...
          CASE
            WHEN c.atendimento_humano_ativo IS NOT TRUE THEN 'AGUARDANDO_ATENDIMENTO'
            WHEN UPPER(COALESCE(lm.quem,'')) IN ('USUARIO','CANDIDATO') THEN 'AGUARDANDO_RECRUTADOR'
            WHEN UPPER(COALESCE(lm.quem,'')) IN ('RECRUTADOR','IA') THEN 'AGUARDANDO_CANDIDATO'
            ELSE 'EM_CONVERSA'
        ...
        ORDER BY
          CASE
            WHEN c.atendimento_humano_ativo IS NOT TRUE THEN 0
            WHEN UPPER(COALESCE(lm.quem,'')) IN ('USUARIO','CANDIDATO') THEN 1
            ELSE 2
          END,
  ...
  app.post('/api/atendimento/candidatos/:id/finalizar-handoff', requireLogin, async(req,res,next)=>{
    ...
    try{
      ...
      const stageResult=await client.query(`SELECT genesis_v16_etapa_retomada_apos_suporte($1) etapa`,[id]);
      ...
      await client.query(`UPDATE candidato_revisoes SET status='CONCLUIDO',decisao='ATENDIMENTO_REALIZADO',decisao_motivo=$2,decidido_por=$3,decidido_em=NOW(),updated_at=NOW() WHERE candidato_id=$1 AND tipo='SUPORTE_FLUXO' AND status='PENDENTE'`,[id,summary||'Atendimento humano concluído e devolvido para a IA.',name]);
      await client.query(`INSERT INTO atendimento_handoff_historico(candidato_id,usuario_id,usuario_nome,etapa_anterior,etapa_retomada,dados_aplicados,documentos_pendentes,resumo,triagem_aplicada) VALUES($1,$2,$3,$4,$5,$6::JSONB,$7::BIGINT[],$8,$9::JSONB)`,[id,req.user?.id||null,name,candidate.etapa,docIds.length?candidate.etapa:resumedStage,JSON.stringify(Object.fromEntries(changes.map((c)=>[c.field,c.after]))),docIds,summary||null,triageApplied?JSON.stringify(triageApplied):null]);
      await client.query(`INSERT INTO eventos(candidato_id,evento,descricao,created_at) VALUES($1,'ATENDIMENTO_HUMANO_FINALIZADO_V16',$2,NOW())`,[id,`Atendimento finalizado por ${name}. ${changes.length} dado(s) aplicado(s); ${triageApplied?'1 resposta de triagem confirmada; ':''}${docIds.length} documento(s) aguardando aplicação automática.`]);
      await client.query('COMMIT');

      let continuation=null;
      if(!docIds.length){
        const updated=await candidateRow(id);
        const message=buildManualContinuationMessage(updated,'');
        try{continuation=await triggerManualCandidateMessage({candidatoId:id,mensagem:message,evento:'HANDOFF_V16_CONTINUACAO',solicitadoPor:name,session});}
        catch(error){
          await pool.query(`UPDATE candidatos SET ia_atendimento_ativo=FALSE,ia_pausada_em=NOW(),ia_pausada_por=$2,ia_pausa_motivo='Falha ao continuar após handoff',atendimento_humano_ativo=TRUE,atendimento_humano_usuario_id=$3,atendimento_humano_nome=$2,atendimento_humano_solicitado=TRUE,atendimento_humano_solicitado_em=NOW(),revisao_pendente=TRUE,revisao_tipo='SUPORTE_FLUXO',revisao_motivo='Falha ao continuar após handoff',updated_at=NOW() WHERE id=$1`,[id,name,req.user?.id||null]).catch(()=>{});
          await pool.query(`INSERT INTO candidato_revisoes(candidato_id,vaga_id,tipo,titulo,motivo,dados) SELECT c.id,c.vaga_id,'SUPORTE_FLUXO','Continuação do atendimento precisa de atenção','A IA foi pausada novamente porque a continuação após o handoff não foi confirmada.',JSONB_BUILD_OBJECT('origem','HANDOFF_V16','erro',$2) FROM candidatos c WHERE c.id=$1 ON CONFLICT DO NOTHING`,[id,String(error.message).slice(0,1000)]).catch(()=>{});
          throw new AtendimentoV16Error(`Os dados foram salvos, mas a continuação não foi confirmada. A IA foi pausada novamente: ${error.message}`,502);
        }
      }
      res.json({sucesso:true,mensagem:docIds.length?'Atendimento finalizado. A IA foi liberada e os documentos serão aplicados automaticamente.':'Atendimento finalizado e devolvido para a IA.',dados_aplicados:changes.map((c)=>c.field),documentos_pendentes:docIds,triagem_aplicada:triageApplied,proxima_etapa:resumedStage,continuacao:continuation});
    }catch(error){try{await client.query('ROLLBACK');}catch{}next(error);}finally{client.release();}
  });
