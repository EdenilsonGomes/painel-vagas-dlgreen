...
function registerProspectingV20({ app, pool, requireAdmin, currentUserName, wahaBaseUrl, wahaApiKey }) {
  ...
  app.post('/api/admin/prospeccao/enriquecer-lote',requireAdmin,async(req,res,next)=>{try{const ids=[...new Set((Array.isArray(req.body?.ids)?req.body.ids:[]).map(id).filter(Boolean))].slice(0,50);if(!ids.length)return res.status(400).json({sucesso:false,erro:'Selecione ao menos um lead.'});const result=await pool.query(`INSERT INTO prospeccao_enriquecimentos(lead_id,status,solicitado_por,solicitado_por_nome) SELECT id,'PENDENTE',$2,$3 FROM prospeccao_leads WHERE id=ANY($1::BIGINT[]) ON CONFLICT(lead_id) DO UPDATE SET status='PENDENTE',ultimo_erro=NULL,solicitado_por=EXCLUDED.solicitado_por,solicitado_por_nome=EXCLUDED.solicitado_por_nome,updated_at=NOW() RETURNING lead_id`,[ids,req.user?.id||null,currentUserName(req)]);await pool.query(`UPDATE prospeccao_leads SET enriquecimento_status='PENDENTE',updated_at=NOW() WHERE id=ANY($1::BIGINT[])`,[ids]);setImmediate(()=>processOneEnrichment().catch(()=>{}));res.json({sucesso:true,enfileirados:result.rowCount,mensagem:`${result.rowCount} lead(s) enviado(s) para enriquecimento.`});}catch(e){next(e);}});

  app.get('/api/admin/prospeccao/enriquecimento/status',requireAdmin,async(_req,res,next)=>{try{const r=await pool.query(`SELECT COUNT(*) FILTER(WHERE status='PENDENTE')::INT AS pendentes,COUNT(*) FILTER(WHERE status='PROCESSANDO')::INT AS processando,COUNT(*) FILTER(WHERE status='CONCLUIDO')::INT AS concluidos,COUNT(*) FILTER(WHERE status='FALHA')::INT AS falhas FROM prospeccao_enriquecimentos`);res.json({sucesso:true,...r.rows[0]});}catch(e){next(e);}});

  app.post('/api/admin/prospeccao/agendar-lote',requireAdmin,async(req,res,next)=>{try{
    const ids=[...new Set((Array.isArray(req.body?.ids)?req.body.ids:[]).map(id).filter(Boolean))].slice(0,25);
  ...
  const crmStages=['NOVO_LEAD','CONTATADO','RESPONDEU','QUALIFICADO','DEMONSTRACAO','PROPOSTA','NEGOCIACAO','GANHO','PERDIDO'];
  const leadStatusForStage={NOVO_LEAD:'NOVO',CONTATADO:'PRIMEIRO_CONTATO',RESPONDEU:'RESPONDEU',QUALIFICADO:'RESPONDEU',DEMONSTRACAO:'REUNIAO',PROPOSTA:'PROPOSTA',NEGOCIACAO:'PROPOSTA',GANHO:'CLIENTE',PERDIDO:'DESCARTADO'};
  async function ensureLeadOpportunity(client,lead){
    let opportunity=(await client.query('SELECT * FROM crm_oportunidades WHERE prospeccao_lead_id=$1 ORDER BY id DESC LIMIT 1 FOR UPDATE',[lead.id])).rows[0];
    if(opportunity)return opportunity;
    let company=(await client.query(`SELECT id FROM crm_empresas WHERE prospeccao_lead_id=$1 OR (LOWER(nome)=LOWER($2) AND COALESCE(website,'')=COALESCE($3,'')) ORDER BY id LIMIT 1`,[lead.id,lead.empresa_nome,lead.website||null])).rows[0];
    if(!company)company=(await client.query(`INSERT INTO crm_empresas(nome,segmento,cidade,estado,website,origem,prospeccao_lead_id) VALUES($1,$2,$3,$4,$5,'PROSPECCAO',$6) RETURNING id`,[lead.empresa_nome,lead.categoria,lead.cidade,lead.estado,lead.website,lead.id])).rows[0];
    return (await client.query(`INSERT INTO crm_oportunidades(crm_empresa_id,titulo,etapa,responsavel_id,origem,prospeccao_lead_id) VALUES($1,$2,'NOVO_LEAD',$3,'PROSPECCAO',$4) RETURNING *`,[company.id,`Prospecção · ${lead.empresa_nome}`,lead.responsavel_id||null,lead.id])).rows[0];
  }
  app.get('/api/admin/prospeccao/leads/:leadId/detalhes-v20',requireAdmin,async(req,res,next)=>{try{
    const leadId=id(req.params.leadId);if(!leadId)return res.status(400).json({sucesso:false,erro:'Lead inválido.'});
    const lead=(await pool.query(`SELECT l.*,co.id AS oportunidade_id,COALESCE(co.etapa,'NOVO_LEAD') AS crm_etapa FROM prospeccao_leads l LEFT JOIN LATERAL(SELECT * FROM crm_oportunidades WHERE prospeccao_lead_id=l.id ORDER BY id DESC LIMIT 1) co ON TRUE WHERE l.id=$1`,[leadId])).rows[0];
    if(!lead)return res.status(404).json({sucesso:false,erro:'Lead não encontrado.'});
    const modelName={PORTAL_GRATIS:'V20 · Portal grátis',DIVULGACAO_CANDIDATOS:'V20 · Divulgação de vagas',AUTOMACAO_RECRUTAMENTO:'V20 · Automação de RH'}[lead.oferta_sugerida]||'V20 · Automação de RH';
    const model=(await pool.query(`SELECT mensagem FROM prospeccao_modelos_mensagem WHERE ativo IS TRUE AND nome=$1 ORDER BY id LIMIT 1`,[modelName])).rows[0];
    const suggested=personalizeTemplate(model?.mensagem||'Olá! Gostaria de conversar sobre como a Gênesis pode apoiar o recrutamento da {empresa}.',lead,currentUserName(req));
    const history=lead.oportunidade_id?(await pool.query(`SELECT tipo,descricao,criado_por_nome,created_at FROM crm_interacoes WHERE oportunidade_id=$1 ORDER BY created_at DESC,id DESC LIMIT 30`,[lead.oportunidade_id])).rows:[];
    res.json({sucesso:true,lead,mensagem_sugerida:suggested,historico:history});
  }catch(e){next(e);}});
  app.patch('/api/admin/prospeccao/leads/:leadId/etapa-crm',requireAdmin,async(req,res,next)=>{const client=await pool.connect();try{
    const leadId=id(req.params.leadId),stage=String(req.body?.etapa||'').toUpperCase();if(!leadId||!crmStages.includes(stage))return res.status(400).json({sucesso:false,erro:'Etapa inválida.'});
    await client.query('BEGIN');const lead=(await client.query('SELECT * FROM prospeccao_leads WHERE id=$1 FOR UPDATE',[leadId])).rows[0];if(!lead){await client.query('ROLLBACK');return res.status(404).json({sucesso:false,erro:'Lead não encontrado.'});}
    const opportunity=await ensureLeadOpportunity(client,lead),from=opportunity.etapa||'NOVO_LEAD';
    await client.query(`UPDATE crm_oportunidades SET etapa=$2,ganho_em=CASE WHEN $2='GANHO' THEN COALESCE(ganho_em,NOW()) ELSE ganho_em END,perdido_em=CASE WHEN $2='PERDIDO' THEN COALESCE(perdido_em,NOW()) ELSE perdido_em END,updated_at=NOW() WHERE id=$1`,[opportunity.id,stage]);
    await client.query('UPDATE prospeccao_leads SET status=$2,responsavel_id=COALESCE(responsavel_id,$3),updated_at=NOW() WHERE id=$1',[leadId,leadStatusForStage[stage],req.user?.id||null]);
    if(from!==stage)await client.query(`INSERT INTO crm_interacoes(oportunidade_id,tipo,descricao,criado_por,criado_por_nome) VALUES($1,'MUDANCA_ETAPA',$2,$3,$4)`,[opportunity.id,`Etapa alterada de ${from} para ${stage} pelo Kanban da Prospecção.`,req.user?.id||null,currentUserName(req)]);
    await client.query('COMMIT');res.json({sucesso:true,etapa:stage,mensagem:'Etapa atualizada no CRM e no histórico.'});
  }catch(e){try{await client.query('ROLLBACK');}catch{}next(e);}finally{client.release();}});
  app.post('/api/admin/prospeccao/leads/:leadId/preparar-mensagem',requireAdmin,async(req,res,next)=>{try{
    const leadId=id(req.params.leadId),message=text(req.body?.mensagem,4000),source=text(req.body?.autorizacao_origem,200),sendNow=req.body?.enviar_agora===true;if(!leadId||!message||!source)return res.status(400).json({sucesso:false,erro:'Mensagem e origem do contato são obrigatórias.'});
    const lead=(await pool.query('SELECT * FROM prospeccao_leads WHERE id=$1',[leadId])).rows[0];if(!lead)return res.status(404).json({sucesso:false,erro:'Lead não encontrado.'});if(lead.nao_contatar)return res.status(409).json({sucesso:false,erro:'Lead marcado como não contatar.'});const phone=digits(lead.telefone_normalizado||lead.telefone);if(!phone)return res.status(409).json({sucesso:false,erro:'WhatsApp do lead inválido.'});
    const existing=await pool.query(`SELECT 1 FROM prospeccao_envios WHERE lead_id=$1 AND status IN ('AGENDADO','PROCESSANDO','ENVIADO') LIMIT 1`,[leadId]);if(existing.rowCount)return res.status(409).json({sucesso:false,erro:'Este lead já possui contato preparado ou enviado.'});
    const when=sendNow?new Date():nextBusinessTime(new Date(Date.now()+120000));const client=await pool.connect();try{await client.query('BEGIN');await client.query(`UPDATE prospeccao_leads SET contato_autorizado=TRUE,contato_autorizado_origem=$2,contato_autorizado_em=NOW(),status=CASE WHEN status='NOVO' THEN 'APROVADO_CONTATO' ELSE status END,responsavel_id=COALESCE(responsavel_id,$3),updated_at=NOW() WHERE id=$1`,[leadId,source,req.user?.id||null]);await client.query(`INSERT INTO prospeccao_envios(lead_id,session_name,telefone,mensagem,status,agendado_para,aprovado_por) VALUES($1,$2,$3,$4,'AGENDADO',$5,$6)`,[leadId,session,phone,message,when.toISOString(),req.user?.id||null]);await client.query('COMMIT');}catch(e){try{await client.query('ROLLBACK');}catch{}throw e;}finally{client.release();}
    res.status(201).json({sucesso:true,mensagem:sendNow?'Envio imediato solicitado à fila protegida.':'Mensagem preparada para a fila protegida.'});
  }catch(e){next(e);}});
