...
(() => {
  ...
  function ensureUi(){
    ...
    menu.innerHTML='<header><div><strong>Atendimentos</strong><small>Conversas que precisam de atenção</small></div><button type="button" data-gfc-refresh class="icon-button" title="Atualizar">↻</button></header><div id="genesisFloatingChatMenuList" class="genesis-floating-chat-menu-list"></div>';
  ...
  function createChat(item){
    ...
    element.querySelector('[data-gfc-min]').addEventListener('click',()=>element.classList.toggle('minimized'));
  ...
  async function openChat(id,{item=null}={}){
    ...
    const existing=local.open.get(Number(id));if(existing){existing.element.classList.remove('minimized');existing.element.querySelector('textarea')?.focus();return;}
