...
(() => {
  ...
  function renderLeads() {
    ...
    tbody.innerHTML = state.leads.map((lead) => {
      ...
      return `<tr data-lead-row="${lead.id}">
        ...
        <td><div class="table-primary"><strong>${safe(fmtPhone(lead.telefone))}</strong><span>${safe(lead.email || 'E-mail não encontrado')}</span></div></td>
        ...
        <td><div class="table-actions">${whatsapp}<button class="button button-ghost compact" data-admin-action="save-lead" data-id="${lead.id}" type="button">Salvar</button><button class="button button-ghost compact" data-admin-action="add-lead-note" data-id="${lead.id}" type="button">Nota</button></div></td>
      </tr>`;
    }).join('');
  }
  ...
  function renderUsers() {
    ...
    container.innerHTML = state.users.map((user) => `<article class="user-card" data-user-card="${user.id}">
      ...
      <div class="user-main"><strong>${safe(user.nome)}</strong><span>@${safe(user.usuario)} · último acesso ${safe(user.ultimo_login_at ? fmtDate(user.ultimo_login_at) : 'ainda não realizado')}</span><small>Criado em ${safe(fmtDate(user.created_at))}</small></div>
      <div class="user-controls"><select data-user-field="perfil"><option value="RECRUTADOR" ${user.perfil === 'RECRUTADOR' ? 'selected' : ''}>Recrutador</option><option value="ADMIN" ${user.perfil === 'ADMIN' ? 'selected' : ''}>Administrador</option></select><label class="mini-check"><input data-user-field="ativo" type="checkbox" ${user.ativo ? 'checked' : ''}> Ativo</label></div>
