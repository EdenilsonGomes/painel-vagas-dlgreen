...
(() => {
  ...
  function renderUsers() {
    ...
    container.innerHTML = state.users.map((user) => `<article class="user-card" data-user-card="${user.id}">
      ...
      <div class="user-controls"><select data-user-field="perfil"><option value="RECRUTADOR" ${user.perfil === 'RECRUTADOR' ? 'selected' : ''}>Recrutador</option><option value="ADMIN" ${user.perfil === 'ADMIN' ? 'selected' : ''}>Administrador</option></select><select data-user-field="empresa_id">${companyOptions(user.empresa_id)}</select><label class="mini-check"><input data-user-field="ativo" type="checkbox" ${user.ativo ? 'checked' : ''}> Ativo</label></div>
