...
(() => {
  ...
  function proximityMeta(value, status) {
    ...
    if (!hasKm) {
      ...
      if (status === 'COORDENADA_COMPARTILHADA') return { label: 'Baixa precisão', className: 'geo-distance-pending', tone: 'pending', hint: 'CEPs diferentes receberam a mesma coordenada da fonte geográfica' };
      ...
      : geo.geo_status === 'SEM_CEP_VAGA' ? 'Cadastre o CEP da vaga para calcular a distância.'
        : geo.geo_status === 'SEM_CEP_CANDIDATO' ? 'O candidato ainda não possui CEP válido.'
          : geo.geo_status === 'SEM_VAGA' ? 'Candidato sem vaga vinculada.'
            : geo.geo_status === 'COORDENADA_COMPARTILHADA' ? 'A fonte geográfica retornou a mesma coordenada para CEPs diferentes. A Gênesis não mostra 0,0 km para evitar uma distância falsa.'
  ...
  function ensureGeoAttribution() {
    if (document.getElementById('geoAttribution')) return;
    const table = document.querySelector('.candidate-data-table');
    if (!table) return;
    const note = document.createElement('small');
    note.id = 'geoAttribution';
    note.className = 'geo-attribution';
    note.style.cssText = 'display:block;margin:8px 4px 0;font-size:11px;line-height:1.4;opacity:.68;';
    note.innerHTML = 'Geolocalização aproximada por BrasilAPI + <span>© OpenStreetMap contributors</span>.';
