...
const viewMeta = {
  ...
  interviews: ['GOOGLE CALENDAR', 'Entrevistas', 'Calendário mensal das entrevistas on-line e links do Google Meet.', 'Atualizar agenda'],
  ...
  monitoring: ['OBSERVABILIDADE', 'Monitoramento', 'Falhas técnicas, documentos pendentes e saúde dos serviços.', 'Atualizar monitoramento'],
  ...
  publications: ['AQUISIÇÃO', 'Portal e grupos', 'Modere publicações e transforme recrutadores e empresas em oportunidades comerciais.', ''],
...
const el = Object.fromEntries([
  ...
  'dashboardUpdatedAt', 'kpiActiveCandidates', 'kpiInterviewsToday', 'kpiHumanPending', 'kpiDocumentFailures', 'kpiStaleCandidates',
  'dashboardAttention', 'dashboardInterviews', 'dashboardHealth', 'dashboardJourneyStarted', 'dashboardJourneyCtps', 'dashboardJourneyApproved', 'dashboardJourneyScheduled',
  ...
  'candidateStatusSelect', 'candidateStageSelect', 'updateCandidateButton',
  ...
  'candidateDocuments', 'candidateTimeline',
...
function dashboardAttentionMeta(item) {
  ...
  if (type === 'DOCUMENTOS_FALHA') return { icon: '!', tone: 'critical', title: `${Number(item.quantidade || 0)} documento(s) com falha`, description: 'Os arquivos brutos estão preservados e podem ser revisados ou reprocessados.', action: 'Revisar', view: 'documents' };
  if (type === 'REVISOES_PENDENTES') return { icon: '◇', tone: 'warning', title: `${Number(item.quantidade || 0)} decisão(ões) aguardando recrutador`, description: 'Exceções de experiência, documentos inconclusivos ou suporte de fluxo.', action: 'Decidir', view: 'reviews' };
  if (type === 'APROVADOS_SEM_HORARIO') return { icon: '◷', tone: '', title: `${Number(item.quantidade || 0)} aprovado(s) ainda sem entrevista`, description: 'A triagem terminou, mas ainda não existe entrevista agendada no Google Calendar.', action: 'Abrir', candidateId: item.candidato_id };
  if (type === 'SEM_RESPOSTA') return { icon: '↻', tone: '', title: `${Number(item.quantidade || 0)} conversa(s) sem resposta há mais de 2 horas`, description: 'O candidato enviou mensagem e ainda não recebeu continuidade automática.', action: 'Abrir', candidateId: item.candidato_id };
  return { icon: '✓', tone: 'success', title: 'Operação estável', description: 'Nenhuma prioridade operacional foi detectada neste momento.', action: 'Detalhes', view: 'monitoring' };
}
...
function renderDashboard() {
  ...
  el.kpiDocumentFailures.textContent = Number(metrics.documentos_falha || 0);
  el.kpiStaleCandidates.textContent = Number(metrics.sem_resposta_2h || 0);
  ...
  const interviews = (data.proximas_entrevistas || []).filter((item) => {
    if (!item.inicio) return false;
    return new Intl.DateTimeFormat('en-CA', { timeZone: 'America/Sao_Paulo' }).format(new Date(item.inicio)) === today;
  }).slice(0, 3);
  el.dashboardInterviews.innerHTML = interviews.length ? interviews.map((item) => `
    <button class="dashboard-agenda-row" data-action="open-candidate" data-id="${item.candidato_id}" type="button">
      <time>${escapeHtml(formatTime(item.inicio))}</time>
      <span><strong>${escapeHtml(item.candidato_nome)}</strong><small>${escapeHtml(item.vaga_nome)}</small></span>
      <b>Meet</b>
    </button>
  `).join('') : emptyState('Nenhuma entrevista hoje', 'Os próximos horários aparecem no calendário.');

  const healthRows = [
    ['n8n', health.n8n || 'Sem atividade'],
    ['WAHA', health.waha || 'Sem atividade'],
    ['Banco', health.banco || 'Online'],
    ['Calendar', health.calendar || 'Sem erro recente'],
  ];
  el.dashboardHealth.innerHTML = healthRows.map(([label, value]) => {
    const attentionState = /atenção|erro|sem atividade/i.test(String(value));
    return `<div><span>${escapeHtml(label)}</span><strong class="${attentionState ? 'attention' : ''}">${escapeHtml(value)}</strong></div>`;
  }).join('');
...
function renderCandidateDrawer(details) {
  ...
  el.candidateStatusSelect.value = ['NOVO','EM_PROCESSO','APROVADO','EM_ADMISSAO','CONTRATADO','ENCERRADO'].includes(c.status) ? c.status : 'EM_PROCESSO';
...
async function updateCandidate() {
  ...
  try {
    ...
    showToast('Estado técnico do candidato corrigido.');
...
function reviewTypeLabel(value) {
  return ({ EXCECAO_EXPERIENCIA: 'Exceção de experiência', REVISAO_DOCUMENTAL: 'Revisão documental', SUPORTE_FLUXO: 'Suporte de fluxo', DIVERGENCIA_DADOS: 'Divergência de dados' })[value] || value;
}
...
function renderReviews() {
  ...
  el.reviewsList.innerHTML = items.length ? items.map((item) => {
    ...
    return `<article class="review-card"><header><div><span class="review-kind">${escapeHtml(reviewTypeLabel(item.tipo))}</span><h3>${escapeHtml(item.candidato_nome)}</h3><p>${escapeHtml(item.vaga_nome || 'Sem vaga vinculada')}</p></div><time>${escapeHtml(formatDate(item.created_at))}</time></header>${metrics}${support}<p class="review-reason">${escapeHtml(item.motivo || item.titulo)}</p><footer>${documentButton}${curriculumButton}<button class="button button-primary" data-review-decision="APROVAR" data-id="${item.id}" type="button">Aprovar e continuar</button>${reprocessButton}${newPdfButton}<button class="button button-danger-soft" data-review-decision="NAO_APROVAR" data-id="${item.id}" type="button">Não aprovar nesta vaga</button></footer></article>`;
...
async function loadCurrentUser() {
  ...
  if (role !== 'ADMIN' && ['audit', 'prospecting', 'publications', 'users'].includes(state.activeView)) setView('dashboard');
}
