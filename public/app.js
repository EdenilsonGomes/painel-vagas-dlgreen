...
const el = Object.fromEntries([
  ...
  'auditSyncPeriod', 'auditCustomRange', 'auditCustomStart', 'auditCustomEnd', 'auditSyncButton', 'auditKpiConversations', 'auditKpiClean', 'auditKpiCritical', 'auditKpiHigh', 'auditKpiScore', 'auditLastSync',
  'auditTrendChart', 'auditTopCategories', 'auditResultCount', 'auditSeverityFilter', 'auditStatusFilter', 'auditSearchInput', 'auditLoading', 'auditEmpty', 'auditProblemsList',
  'auditProblemDialog', 'auditProblemTitle', 'auditProblemSubtitle', 'closeAuditProblemButton', 'auditProblemContent', 'auditReviewObservation',
...
async function openVacancyDialog(vacancy = null, duplicate = false) {
  ...
  const defaults = {
    ...
    quantidade_vagas: 1, idade_minima: 25, experiencia_minima_meses: 0, exigir_experiencia_compativel: true, publicar_portal: true, destaque_portal: false, canal_candidatura: 'WHATSAPP_GENESIS',
...
function vacancyFormPayload() {
  ...
  return {
    ...
    escolaridade_minima: data.get('escolaridade_minima'), idade_minima: data.get('idade_minima') || 25,
...
function handleDelegatedAction(event) {
  const target = event.target.closest('[data-action], [data-vacancy-action], [data-candidate-action], [data-task-action], [data-monitor-action], [data-go-view], [data-audit-open], [data-template-apply], [data-template-edit], [data-template-duplicate], [data-template-delete]');
  if (!target) return;
