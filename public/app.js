...
function renderCandidates() {
  ...
  elements.candidatesTableBody.innerHTML = candidates.map((candidate) => {
    ...
    const personality = candidate.personalidade_resumo
      ? escapeHtml(candidate.personalidade_resumo)
      : 'Ainda não analisado';

    const documentsCount = Number(candidate.quantidade_documentos || 0);
    const interview = candidate.entrevista_inicio
    ...
    return `
      <tr>
        ...
        <td>
          <div class="personality-preview" title="${personality}">
            ${personality}
          </div>
        </td>
        <td>
          <button class="link-button" type="button" data-candidate-action="details" data-id="${escapeHtml(candidate.id)}">
        ...
        <td class="row-actions">
          <button class="button button-small button-secondary" type="button" data-candidate-action="details" data-id="${escapeHtml(candidate.id)}">
            Ver detalhes
          </button>
        </td>
      </tr>
...
function handleCandidateTableClick(event) {
  const button = event.target.closest('[data-candidate-action="details"]');
  if (!button) return;
  openCandidateDetails(button.dataset.id);
}
