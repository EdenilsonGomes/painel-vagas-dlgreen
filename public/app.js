...
function renderDashboard() {
  ...
  elements.dashboardActivity.innerHTML = activities.length ? activities.map((item) => `
    <article class="activity-item">
      ...
      <div><strong>${escapeHtml(actionLabel(item.acao))}: ${escapeHtml(item.nome || item.telefone || `Candidato #${item.candidato_id || ''}`)}</strong><small>${escapeHtml(formatDate(item.created_at))}${Array.isArray(item.campos_alterados) && item.campos_alterados.length ? ` · ${escapeHtml(item.campos_alterados.slice(0, 4).join(', '))}` : ''}</small></div>
    </article>
...
async function openCandidateDetails(id) {
  ...
  try {
    ...
    elements.detailCandidateTriage.textContent = candidate.observacao_triagem || candidate.motivo_reprovacao || 'Ainda não analisado.';
    ...
    if (rejected) {
      elements.detailCandidateRejectionReason.textContent = candidate.motivo_reprovacao || 'A reprovação foi registrada sem um motivo detalhado.';
      elements.detailCandidateRejectionObservation.textContent = candidate.observacao_triagem || 'Sem observação complementar da análise.';
    }
