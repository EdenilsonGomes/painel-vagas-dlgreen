...
const viewMeta = {
  ...
  atendimentos: ['ATENDIMENTO HUMANO', 'Atendimentos', 'Veja quem está esperando o recrutador e quem está esperando o candidato.', 'Atualizar atendimentos'],
...
async function loadCurrentView(force = false) {
  try {
    ...
    if (state.activeView === 'atendimentos') await window.GenesisAtendimentosV16?.load(force);
...
function renderCandidateTable(candidates) {
  ...
  el.candidatesTableBody.innerHTML = candidates.map((c) => {
    ...
    return `<tr data-candidate-row="${c.id}" tabindex="0" role="button" aria-label="Abrir perfil de ${escapeHtml(c.nome || 'candidato')}">
      ...
      <td><span class="badge ${badgeClass(c.status)}">${escapeHtml(statusLabels[c.status] || c.status || 'Não informado')}</span>${candidateHumanServiceBadge(c)}<div class="primary-cell"><span>${escapeHtml(stageLabels[c.etapa] || c.etapa || 'Etapa não informada')}</span></div></td>
