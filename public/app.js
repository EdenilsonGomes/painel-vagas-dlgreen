...
const el = Object.fromEntries([
  ...
  'dashboardAttention', 'dashboardInterviews', 'dashboardFunnel', 'dashboardJourneyStarted', 'dashboardJourneyCtps', 'dashboardJourneyApproved', 'dashboardJourneyScheduled',
  ...
  'candidateConversation', 'candidateConversationWhatsappButton', 'candidateDocuments', 'candidateTimeline',
...
function setView(name) {
  if (!currentUserIsAdmin() && ['audit', 'prospecting', 'publications', 'monitoring', 'demos', 'users'].includes(name)) name = 'dashboard';
...
function renderDashboard() {
  ...
  el.kpiCritical.textContent = Number(metrics.documentos_falha || 0) + Number(metrics.sem_resposta_2h || 0);
...
function renderCandidateDrawer(details) {
  ...
  el.candidateConversationWhatsappButton.href = phoneDigits ? `https://wa.me/${phoneDigits}` : '#';
...
function renderCandidateConversation(messages) {
  el.candidateConversation.innerHTML = messages.length ? messages.map((item) => {
    ...
    const label = incoming ? 'Candidato' : (author === 'IA' ? 'Evelyn' : 'Equipe Genesis');
    return `<article class="conversation-message ${incoming ? 'incoming' : 'outgoing'}"><div><strong>${escapeHtml(label)}</strong><time>${escapeHtml(formatDate(item.created_at))}</time></div><p>${escapeHtml(item.mensagem || 'Mensagem sem conteúdo')}</p></article>`;
...
async function loadCurrentUser() {
  ...
  if (role !== 'ADMIN' && ['audit', 'prospecting', 'publications', 'monitoring', 'demos', 'users'].includes(state.activeView)) setView('dashboard');
}
