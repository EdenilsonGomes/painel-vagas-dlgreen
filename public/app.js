...
const state = {
  ...
  companies: [],
  candidates: [],
  candidateSummary: { total: 0, em_processo: 0, aprovados: 0, reprovados: 0 },
...
const viewMeta = {
  dashboard: ['OPERAÇÃO', 'Visão geral', 'Prioridades e movimentações da operação em tempo real.', '+ Nova vaga'],
...
const el = Object.fromEntries([
  ...
  'kpiInterviews', 'kpiAlerts', 'kpiUnlinked', 'dashboardAlertCount', 'dashboardAlerts',
  'dashboardFunnel', 'dashboardInterviews', 'dashboardRecentCandidates', 'dashboardActivity',
  ...
  'candidateKpiApproved', 'candidateKpiRejected', 'candidateTableContainer', 'candidateKanbanContainer',
  ...
  'monitorErrors', 'monitorHealth', 'monitorLogs', 'globalSearchDialog', 'globalSearchInput',
  ...
  'saveNewCandidateButton', 'newCandidateVacancy', 'newCandidateError', 'toast',
...
function badgeClass(status) {
  ...
  if (['ATIVA', 'APROVADO', 'CONTRATADO', 'AGENDADA', 'CONCLUIDA'].includes(value)) return 'badge-active';
...
function renderDashboard() {
  ...
  el.kpiInterviews.textContent = Number(metrics.entrevistas_7_dias || 0);
  el.kpiAlerts.textContent = Number(data.total_alertas || alerts.length);
  el.kpiUnlinked.textContent = Number(metrics.entradas_sem_candidato || 0);
  el.dashboardAlertCount.textContent = Number(data.total_alertas || alerts.length);

  el.dashboardAlerts.innerHTML = alerts.length ? alerts.map((alert) => `
    <article class="action-card ${String(alert.severidade || '').toUpperCase() === 'CRITICO' || String(alert.severidade || '').toUpperCase() === 'ALTO' ? 'high' : 'medium'}">
      <div class="action-icon">!</div>
      <div class="action-copy">
        <strong>${escapeHtml(alert.titulo || 'Alerta')}</strong>
        <span>${escapeHtml(alert.descricao || 'Ação pendente')}</span>
        <small>${escapeHtml(formatDate(alert.created_at))}</small>
      </div>
      <div class="action-buttons">
        ${alert.candidato_id ? `<button data-action="open-candidate" data-id="${alert.candidato_id}" type="button">Abrir</button>` : ''}
        <button data-action="resolve-alert" data-key="${escapeHtml(alert.chave)}" type="button">Resolver</button>
      </div>
    </article>
  `).join('') : emptyState('Operação em dia', 'Nenhum alerta pendente no momento.');

  const funnelSource = candidates.length ? candidates : (data.candidatos_recentes || []);
  const stages = [
    ['Novos contatos', (c) => ['PRIMEIRO_CONTATO', 'ESCOLHENDO_VAGA'].includes(c.etapa)],
    ['Documentos', (c) => ['AGUARDANDO_CTPS_CEP', 'AGUARDANDO_CTPS', 'AGUARDANDO_CEP'].includes(c.etapa)],
    ['Em análise', (c) => c.etapa === 'ANALISANDO_DOCUMENTOS'],
    ['Aprovados', (c) => ['APROVADO', 'CONTRATADO'].includes(c.status)],
    ['Entrevista', (c) => ['GERANDO_OPCOES_ENTREVISTA', 'ESCOLHENDO_HORARIO', 'ENTREVISTA_AGENDADA'].includes(c.etapa)],
  ];
  const counts = stages.map(([label, fn]) => [label, funnelSource.filter(fn).length]);
  const max = Math.max(1, ...counts.map((item) => item[1]));
  el.dashboardFunnel.innerHTML = counts.map(([label, count]) => `
    <div class="funnel-row"><span>${escapeHtml(label)}</span><div class="funnel-bar"><i style="width:${Math.max(count ? 7 : 0, count / max * 100)}%"></i></div><strong>${count}</strong></div>
  `).join('');
  ...
  el.dashboardRecentCandidates.innerHTML = recent.length ? recent.map((item) => `
    <button class="compact-item" data-action="open-candidate" data-id="${item.id}" type="button">
      <span class="compact-avatar">${escapeHtml(initials(item.nome || item.telefone))}</span>
      <span><strong>${escapeHtml(item.nome || 'Nome não informado')}</strong><small>${escapeHtml(item.vaga_nome || 'Sem vaga')} · ${escapeHtml(statusLabels[item.status] || item.status || 'Novo')}</small></span>
      <time>${escapeHtml(formatDate(item.updated_at, { dateOnly: true }))}</time>
    </button>
  `).join('') : emptyState('Nenhum candidato registrado');

  const activity = data.atividades || [];
  el.dashboardActivity.innerHTML = activity.length ? activity.map((item) => {
    const fields = Array.isArray(item.campos_alterados) ? item.campos_alterados.filter(Boolean) : [];
    const subject = item.nome || item.telefone || `Candidato #${item.candidato_id || ''}`;
    const action = item.acao === 'ADICIONADO' ? 'Candidato adicionado' : item.acao === 'REMOVIDO' ? 'Candidato removido' : 'Cadastro atualizado';
    return `<article class="activity-row"><span class="activity-dot"></span><div><strong>${escapeHtml(action)} · ${escapeHtml(subject)}</strong><small>${escapeHtml(formatDate(item.created_at))}${fields.length ? ` · Campos: ${escapeHtml(fields.slice(0, 6).join(', '))}` : ''}</small></div></article>`;
  }).join('') : emptyState('Ainda não existem registros de auditoria.');
}
...
function candidateMatches(candidate) {
  ...
  const statusMatch = state.candidateStatus === 'TODOS'
    ...
    || (state.candidateStatus === 'APROVADO' && ['APROVADO', 'CONTRATADO'].includes(status))
...
function kanbanGroup(candidate) {
  ...
  if (['APROVADO', 'CONTRATADO'].includes(candidate.status)) return 'approved';
...
function renderCandidateKanban(candidates) {
  const columns = [
    ...
    ['approved', 'Aprovados'], ['interview', 'Entrevista'], ['rejected', 'Reprovados'],
  ];
...
function renderCandidateDrawer(details) {
  ...
  el.candidateRejectionReason.textContent = c.motivo_reprovacao || 'A reprovação foi registrada sem motivo detalhado.';
  el.candidateRejectionObservation.textContent = c.observacao_triagem || 'Sem observação complementar.';
...
function renderMonitoring() {
  ...
  `).join('') : emptyState('Nenhum erro registrado', 'O workflow de monitoramento preencherá esta área quando houver falhas.');
...
function bindEvents() {
  ...
  el.downloadPrimaryPromotionButton.addEventListener('click', () => downloadSvgAsPng(state.promotion?.imagem_data_url, String(state.promotion?.nome_arquivo || 'vaga').replace(/\.svg$/i, '.png')));
  el.downloadDetailsPromotionButton.addEventListener('click', () => downloadSvgAsPng(state.promotion?.imagem_detalhes_data_url, String(state.promotion?.nome_arquivo_detalhes || 'vaga-detalhes').replace(/\.svg$/i, '.png')));
