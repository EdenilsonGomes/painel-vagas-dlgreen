...
const viewMeta = {
  dashboard: ['OPERAÇÃO', 'Visão geral', 'Decisões humanas, entrevistas e saúde do fluxo determinístico.', '+ Nova vaga'],
...
const el = Object.fromEntries([
  ...
  'documentsList', 'monitorKpiEntries', 'monitorKpiErrors', 'monitorKpiDocs',
...
async function loadCurrentView(force = false) {
  try {
    if (state.activeView === 'dashboard') { await Promise.all([loadDashboard(force), loadReviews(force)]); }
...
function renderDashboard() {
  ...
  if (el.dashboardFunnel) el.dashboardFunnel.innerHTML = normalized.length ? normalized.map((item) => `
...
function handleDelegatedAction(event) {
  const target = event.target.closest('[data-action], [data-vacancy-action], [data-candidate-action], [data-task-action], [data-monitor-action], [data-go-view], [data-audit-open], [data-audit-candidate-profile], [data-template-apply], [data-template-edit], [data-template-duplicate], [data-template-delete], [data-audit-toggle], [data-audit-rescue-candidate], [data-review-decision], [data-calendar-date]');
  if (!target) return;
