...
const el = Object.fromEntries([
  ...
  'vacancyViewDialog', 'vacancyViewTitle', 'vacancyViewSubtitle', 'closeVacancyViewButton',
  'vacancyViewInterested1d', 'vacancyViewInterested7d', 'vacancyViewInterested30d', 'vacancyViewDaysOpen',
  'vacancyViewCards', 'vacancyViewWhatsappText', 'vacancyViewFacebookText', 'vacancyViewImage', 'vacancyViewDownloadButton',
...
function renderVacancies() {
  ...
  el.vacancyKpiInterested.textContent = state.vacancies.reduce((sum, v) => sum + Number(v.total_interessados_periodo || 0), 0);
  el.vacancyKpiInProcess.textContent = state.vacancies.reduce((sum, v) => sum + Number(v.candidatos_funil_periodo || 0), 0);
  el.vacancyKpiApproved.textContent = state.vacancies.reduce((sum, v) => sum + Number(v.candidatos_aprovados_periodo || 0), 0);
  ...
  el.vacanciesTableBody.innerHTML = vacancies.map((v) => {
    ...
    const gains = calculateApproximateGains(v);
    return `
      <tr>
        ...
        <td><div class="remuneration-lines"><strong>${escapeHtml(formatMoney(v.salario))}</strong><small>Ganhos aprox. ${escapeHtml(formatMoney(gains))}</small>${insalubrity}</div></td>
        <td><div class="metric-chips"><span class="metric-chip">Escolheram ${Number(v.total_interessados_periodo || 0)}</span><span class="metric-chip">No funil ${Number(v.candidatos_funil_periodo || 0)}</span><span class="metric-chip good">Aprovados ${Number(v.candidatos_aprovados_periodo || 0)}</span><span class="metric-chip bad">Reprovados ${Number(v.candidatos_reprovados_periodo || 0)}</span></div></td>
        <td><div class="status-stack"><span class="badge ${badgeClass(v.status)}">${escapeHtml(vacancyStatusLabels[v.status] || v.status)}</span>${v.publicar_portal ? '<span class="portal-chip">Portal</span>' : '<span class="portal-chip muted">Interna</span>'}</div></td>
        <td>${escapeHtml(String(calculateDaysOpen(v)))} dia(s)</td>
        <td><div class="row-actions">${v.status === 'ATIVA' && v.publicar_portal && publicVacancyUrl(v) ? `<a class="portal-action" href="${escapeHtml(publicVacancyUrl(v))}" target="_blank" rel="noopener">Abrir portal</a>` : ''}<button data-vacancy-action="view" data-id="${v.id}">Ver vaga</button><button data-vacancy-action="edit" data-id="${v.id}">Editar</button><button data-vacancy-action="duplicate" data-id="${v.id}">Duplicar</button><button class="primary" data-vacancy-action="promote" data-id="${v.id}">Divulgar</button>${v.status === 'ATIVA' ? `<button data-vacancy-action="status" data-status="PAUSADA" data-id="${v.id}">Pausar</button>` : `<button data-vacancy-action="status" data-status="ATIVA" data-id="${v.id}">Ativar</button>`}<button class="danger" data-vacancy-action="delete" data-id="${v.id}">Excluir</button></div></td>
      </tr>`;
  }).join('');
}
...
async function openVacancyView(id) {
  const [detail, day1, day7, day30, promotion] = await Promise.all([
    api(`/api/vagas/${encodeURIComponent(id)}`),
    api(`/api/vagas?periodo=1D`),
    api(`/api/vagas?periodo=7D`),
    api(`/api/vagas?periodo=30D`),
    api(`/api/vagas/${encodeURIComponent(id)}/divulgacao`, { method: 'POST', body: '{}' }),
  ]);
  const vacancy = detail.vaga || vacancyById(id);
  const findById = (response) => (response.vagas || []).find((item) => String(item.id) === String(id)) || {};
  const stats1 = findById(day1);
  const stats7 = findById(day7);
  const stats30 = findById(day30);
  const div = promotion.divulgacao || {};
  el.vacancyViewTitle.textContent = vacancy?.titulo || 'Resumo da vaga';
  el.vacancyViewSubtitle.textContent = `${vacancy?.codigo || ''} · ${vacancy?.empresa_nome || ''}`;
  el.vacancyViewInterested1d.textContent = Number(stats1.total_interessados_periodo || 0);
  el.vacancyViewInterested7d.textContent = Number(stats7.total_interessados_periodo || 0);
  el.vacancyViewInterested30d.textContent = Number(stats30.total_interessados_periodo || 0);
  el.vacancyViewDaysOpen.textContent = calculateDaysOpen(vacancy);
  ...
  el.vacancyViewCards.innerHTML = `
    <article class="info-card"><strong>Empresa</strong><span>${escapeHtml(vacancy?.empresa_nome || 'Não informada')}</span></article>
    <article class="info-card"><strong>Sexo</strong><span>${escapeHtml(vacancy?.sexo || 'UNISSEX')}</span></article>
    <article class="info-card"><strong>Local</strong><span>${escapeHtml(location)}</span></article>
    <article class="info-card"><strong>Jornada</strong><span>${escapeHtml(schedule || 'Não informada')}</span></article>
    <article class="info-card"><strong>Salário</strong><span>${escapeHtml(formatMoney(vacancy?.salario))}</span></article>
    <article class="info-card"><strong>Status</strong><span>${escapeHtml(vacancyStatusLabels[vacancy?.status] || vacancy?.status || '')}</span></article>`;
  el.vacancyViewWhatsappText.value = div.whatsapp_texto || '';
  el.vacancyViewFacebookText.value = div.facebook_texto || '';
  el.vacancyViewImage.src = div.imagem_png_url || div.imagem_data_url || '';
  el.vacancyViewDownloadButton.onclick = () => {
    if (div.imagem_png_url) window.location.assign(div.imagem_png_url);
    else downloadSvgAsPng(div.imagem_data_url, String(div.nome_arquivo || 'vaga').replace(/\.svg$/i, '.png'));
  };
...
async function openVacancyDialog(vacancy = null, duplicate = false) {
  ...
  const defaults = {
    ...
    quantidade_vagas: 1, experiencia_minima_meses: 0, exigir_experiencia_compativel: true, publicar_portal: true, destaque_portal: false, canal_candidatura: 'WHATSAPP_GENESIS',
  };
...
async function savePostInterviewDecision() {
  ...
  if (decisao === 'REPROVADO_POS_ENTREVISTA' && !motivo) {
    el.postInterviewReasonField.classList.remove('hidden');
  ...
  try {
    const result = await api(`/api/candidatos/${state.selectedCandidateId}/decisao-pos-entrevista`, {
      method: 'POST',
      body: JSON.stringify({ decisao, motivo, observacao }),
    });
...
async function loadCurrentUser() {
  ...
  if (role !== 'ADMIN' && ['prospecting', 'users'].includes(state.activeView)) setView('dashboard');
}
