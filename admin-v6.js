...
function registerAdminV6({ app, pool, requireAdmin, currentUserName }) {
  ...
  app.get('/api/admin/usuarios', requireAdmin, async (req, res, next) => {
    try {
      const result = await pool.query(`
        SELECT id, usuario, nome, perfil, ativo, deve_trocar_senha, ultimo_login_at, created_at, updated_at
        FROM app_usuarios
        ORDER BY ativo DESC, perfil ASC, nome ASC
      `);
  ...
  app.post('/api/admin/usuarios', requireAdmin, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`
        INSERT INTO app_usuarios (usuario, senha_hash, nome, perfil, ativo, criado_por)
        VALUES ($1, $2, $3, $4, $5, $6)
        RETURNING id, usuario, nome, perfil, ativo, created_at
      `, [usuario, senhaHash, parsed.data.nome, parsed.data.perfil, parsed.data.ativo, req.user.id]);
  ...
  app.put('/api/admin/usuarios/:id', requireAdmin, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`
        UPDATE app_usuarios
        SET nome = $1, perfil = $2, ativo = $3, updated_at = NOW()
        WHERE id = $4
        RETURNING id, usuario, nome, perfil, ativo, updated_at
      `, [parsed.data.nome, parsed.data.perfil, parsed.data.ativo, id]);
  ...
  app.get('/api/admin/prospeccao/leads', requireAdmin, async (req, res, next) => {
    try {
      ...
      `);
      res.json({ sucesso: true, leads: result.rows, resumo: summary.rows[0] });
    } catch (error) { next(error); }
  });
