...
function registerAdminV6({ app, pool, requireAdmin, currentUserName }) {
    ...
    try {
      const result = await pool.query(`
        SELECT u.id,u.usuario,u.nome,u.perfil,u.ativo,u.empresa_id,e.nome AS empresa_nome,
          u.deve_trocar_senha,u.ultimo_login_at,u.created_at,u.updated_at
  ...
  app.post('/api/admin/usuarios', requireAdmin, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`
        INSERT INTO app_usuarios (usuario, senha_hash, nome, perfil, empresa_id, ativo, criado_por)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, usuario, nome, perfil, empresa_id, ativo, created_at
      `, [usuario, senhaHash, parsed.data.nome, parsed.data.perfil, parsed.data.empresa_id, parsed.data.ativo, req.user.id]);
  ...
  app.put('/api/admin/usuarios/:id', requireAdmin, async (req, res, next) => {
    try {
      ...
      const result = await pool.query(`
        UPDATE app_usuarios
        SET nome = $1, perfil = $2, empresa_id=$3, ativo = $4, updated_at = NOW()
        WHERE id = $5
        RETURNING id, usuario, nome, perfil, empresa_id, ativo, updated_at
      `, [parsed.data.nome, parsed.data.perfil, parsed.data.empresa_id, parsed.data.ativo, id]);
