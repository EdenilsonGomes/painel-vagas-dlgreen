-- Genesis IA V13.4 — fluxo mais maleável, previsível e econômico
-- Patch incremental: não altera tabelas, não apaga dados e não muda credenciais.

BEGIN;

CREATE OR REPLACE FUNCTION public.genesis_v13_nome_valido(p_texto TEXT)
RETURNS BOOLEAN
LANGUAGE plpgsql
IMMUTABLE
PARALLEL SAFE
...
DECLARE
  ...
  normalizado TEXT := genesis_v13_normalizar_texto(p_texto);
  palavras INTEGER;
BEGIN
  IF CHAR_LENGTH(bruto) NOT BETWEEN 2 AND 80 THEN RETURN FALSE; END IF;
  IF bruto ~ '[0-9?¿!@#$%&*=+_/]' THEN RETURN FALSE; END IF;
  IF CHAR_LENGTH(REGEXP_REPLACE(bruto, '[^[:alpha:]]', '', 'g')) < 2 THEN RETURN FALSE; END IF;
  palavras := COALESCE(ARRAY_LENGTH(REGEXP_SPLIT_TO_ARRAY(normalizado, '[[:space:]]+'), 1), 0);
  IF palavras < 1 OR palavras > 6 THEN RETURN FALSE; END IF;
  IF normalizado IN ('SIM','NAO','PODE','SIM PODE','OK','QUERO','TENHO INTERESSE','BOM DIA','BOA TARDE','BOA NOITE') THEN RETURN FALSE; END IF;
  IF normalizado ~ '(HORARIO|VAGA|SALARIO|BENEFICIO|ESCALA|LOCAL|ENDERECO|CANDIDAT|EMPREGO|TRABALHO|CTPS|CURRICULO|NAO TENHO|QUERO SABER|TEM |QUAL |COMO |ONDE )' THEN RETURN FALSE; END IF;
  RETURN TRUE;
END;
$$;

CREATE OR REPLACE FUNCTION public.genesis_v13_resposta_duvida_vaga(p_vaga_id BIGINT, p_mensagem TEXT)
RETURNS TEXT
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  n TEXT := genesis_v13_normalizar_texto(p_mensagem);
  v_titulo TEXT;
  v_salario NUMERIC;
  v_beneficios TEXT;
  v_local TEXT;
...
BEGIN
  IF p_vaga_id IS NULL THEN RETURN NULL; END IF;
  SELECT
    vg.titulo,
    vg.salario,
    COALESCE(NULLIF(vg.beneficios,''),'benefícios a confirmar com o recrutador'),
    COALESCE(NULLIF(CONCAT_WS(' · ',NULLIF(vg.endereco_referencia,''),NULLIF(vg.bairro,''),NULLIF(vg.cidade,''),NULLIF(vg.estado,'')),''),'local a confirmar'),
    COALESCE(NULLIF(CONCAT_WS(' · ',NULLIF(vg.escala,''),NULLIF(vg.horario,'')),''),'jornada a confirmar'),
    COALESCE(NULLIF(vg.requisitos_obrigatorios,''),'requisitos não informados'),
    COALESCE(vg.experiencia_minima_meses,0)
  INTO v_titulo,v_salario,v_beneficios,v_local,v_jornada,v_requisitos,v_experiencia
  FROM vagas vg WHERE vg.id=p_vaga_id LIMIT 1;
  IF NOT FOUND THEN RETURN NULL; END IF;

  IF n ~ '(SALARIO|REMUNERACAO|QUANTO PAGA|VALOR)' THEN
    RETURN FORMAT('Sobre a vaga %s: o salário é %s.',COALESCE(v_titulo,'selecionada'),CASE WHEN v_salario IS NULL THEN 'a confirmar' ELSE 'R$ '||TO_CHAR(v_salario,'FM999G999G990D00') END);
  ELSIF n ~ '(BENEFICIO|VALE REFEICAO|VALE ALIMENTACAO|VR|VA)' THEN
    RETURN FORMAT('Sobre a vaga %s: %s.',COALESCE(v_titulo,'selecionada'),v_beneficios);
  ELSIF n ~ '(HORARIO|ESCALA|JORNADA|12X36|6X1|ENTRADA|SAIDA)' THEN
    RETURN FORMAT('Sobre a vaga %s: a jornada informada é %s.',COALESCE(v_titulo,'selecionada'),v_jornada);
  ELSIF n ~ '(LOCAL|ENDERECO|BAIRRO|CIDADE|REGIAO|ZONA |ONDE FICA|ONDE E)' THEN
    RETURN FORMAT('Sobre a vaga %s: o local informado é %s.',COALESCE(v_titulo,'selecionada'),v_local);
  ELSIF n ~ '(REQUISITO|EXPERIENCIA|PRECISA TER|TEMPO DE CARTEIRA)' THEN
    RETURN FORMAT('Sobre a vaga %s: experiência mínima de %s mês(es). Requisitos: %s.',COALESCE(v_titulo,'selecionada'),v_experiencia,v_requisitos);
  ELSIF n ~ '(ENTREVISTA|GOOGLE MEET|MEET|PRESENCIAL|ONLINE)' THEN
    RETURN 'A entrevista é on-line pelo Google Meet. Depois da aprovação documental, você escolhe um horário disponível e recebe o link nesta conversa.';
  END IF;
  RETURN NULL;
END;
$$;
...
BEGIN
  ...
  -- Respostas como '2 não' ou '1 quero seguir' usam o primeiro número, sem chamar IA.
  IF canon IS NULL AND n ~ '^([0-9]+)( .*)?$' AND etapa IN ('AGUARDANDO_INTENCAO','RECRUTADOR_MENU','DUVIDAS_GERAIS','DUVIDAS_VAGA','ESCOLHENDO_VAGA','AGUARDANDO_ACAO_VAGA','AGUARDANDO_EXPERIENCIA','AGUARDANDO_TEMPO_DESLOCAMENTO','AGUARDANDO_CONFIRMACAO_CHEGADA','AGUARDANDO_ESCOLHA_HORARIO','NAO_APTO_NESTA_VAGA') THEN
    canon := (REGEXP_MATCH(n,'^([0-9]+)'))[1];
  END IF;
  ...
  contexto:=FORMAT('Etapa atual: %s. Pergunta: %s. Interprete somente entre as opções exibidas e devolva apenas a opção permitida. Não responda à pergunta, não altere etapas e não tome decisões.',etapa,COALESCE(ctx,genesis_chatbot_v1_pergunta_atual(etapa,c.vaga_id)));
  RETURN NEXT;
END;
$_$;
...
BEGIN
  ...
  -- Ao escolher outra vaga, a CTPS já armazenada precisa ser reaplicada aos novos requisitos.
  -- Isso evita retornar para AGUARDANDO_ACAO_VAGA e entrar em loop.
  IF UPPER(COALESCE(v_documento_status, '')) IN ('CONCLUIDO','PROCESSADO','APROVADO','REPROVADO')
     AND c.aprovado IS NULL THEN
    RETURN 'PROCESSANDO_CTPS';
  END IF;
...
BEGIN
  ...
  IF p_etapa = 'PERGUNTAS_VAGA' THEN
    ...
    RETURN CASE WHEN genesis_triagem_v13_pergunta_atual(v_candidato_id, p_vaga_id) IS NULL THEN '' ELSE E'Passo 2 de 4 — Requisitos da vaga\n\n' || genesis_triagem_v13_pergunta_atual(v_candidato_id, p_vaga_id) END;
  END IF;
  ...
  RETURN CASE p_etapa
    ...
    WHEN 'AGUARDANDO_ACAO_VAGA' THEN
      E'Responda com o número ou uma frase curta:\n\n1 — Seguir com a candidatura\n2 — Tirar uma dúvida sobre a vaga\n3 — Ver outra vaga'
    ...
    WHEN 'AGUARDANDO_NOME' THEN E'Passo 1 de 4 — Identificação\n\nQual é o seu primeiro nome?\nExemplo: Maria'
    WHEN 'AGUARDANDO_EXPERIENCIA' THEN FORMAT(
      E'Passo 2 de 4 — Vaga e deslocamento\n\nEsta vaga exige %s mês(es) de experiência comprovada em carteira. Você possui essa experiência?\n\n1 — Sim\n2 — Não\n3 — Não tenho certeza\n\nMesmo respondendo não, sua CTPS será analisada antes da decisão.',
      v_experiencia_minima_meses
    )
    WHEN 'AGUARDANDO_TEMPO_DESLOCAMENTO' THEN FORMAT(
      E'Passo 2 de 4 — Vaga e deslocamento\n\nA vaga fica em %s. Aproximadamente quanto tempo você levaria para chegar?\n\n1 — Até 30 minutos\n2 — De 30 minutos a 1 hora\n3 — De 1 hora a 1 hora e 30 minutos\n4 — Mais de 1 hora e 30 minutos\n5 — Não sei informar',
      v_endereco
    )
    WHEN 'AGUARDANDO_CONFIRMACAO_CHEGADA' THEN FORMAT(
      E'Passo 2 de 4 — Vaga e deslocamento\n\nConsiderando o horário de entrada %s, você consegue chegar antes do início do expediente?\n\n1 — Sim\n2 — Não\n3 — Preciso verificar',
      v_horario
    )
    WHEN 'AGUARDANDO_CEP' THEN E'Passo 3 de 4 — Localização\n\nPara validar a região e encontrar futuras oportunidades próximas, envie seu CEP com 8 números.\n\nExemplo: 04345010'
    WHEN 'AGUARDANDO_CTPS' THEN E'Passo 4 de 4 — Documentação\n\nAgora envie sua Carteira de Trabalho Digital completa como Documento PDF.\n\nNo aplicativo CTPS Digital, acesse Contratos, escolha Enviar Carteira de Trabalho, selecione o documento completo e envie aqui como PDF. Fotos e capturas de tela não são aceitas.\n\nCaso ainda não saiba gerar o documento, escreva: não tenho CTPS.'
...
BEGIN
  ...
  resposta:='Currículo recebido e salvo no seu cadastro. ✅\n\nEle será usado como apoio na avaliação, mas não substitui a CTPS Digital.' || CASE WHEN c.etapa::TEXT='AGUARDANDO_CTPS' THEN E'\n\nPara concluir esta candidatura, ainda preciso da CTPS Digital completa em PDF.' ELSE E'\n\nVamos continuar de onde paramos:\n\n'||genesis_chatbot_v1_pergunta_atual(c.etapa::TEXT,c.vaga_id) END;
...
BEGIN
  ...
  resposta:=CASE WHEN UPPER(COALESCE(p_tipo,'')) LIKE '%AUDIO%' THEN 'No momento, o atendimento automático funciona por texto. Responda com o número da opção ou escreva uma frase curta.' ELSE 'Recebi uma imagem ou arquivo que não consigo analisar com segurança. Para currículo ou CTPS, envie o documento como arquivo PDF. Para continuar a conversa, responda por texto.' END || E'\n\n' || genesis_chatbot_v1_pergunta_atual(c.etapa::TEXT,c.vaga_id);
  INSERT INTO mensagens(candidato_id,quem,mensagem,contexto_snapshot,created_at)
...
BEGIN
  ...
  IF FOUND THEN
    ...
    END IF;
    resposta:='Nesta demonstração, responda por texto. CTPS e currículo são aceitos somente como Documento PDF; áudios, fotos e capturas de tela não são processados.';
    INSERT INTO genesis_demo_mensagens(demo_id,contato_id,quem,mensagem,origem)
...
BEGIN
  ...
  IF codigo IS NOT NULL THEN
    ...
    IF vaga_escolhida IS NOT NULL THEN
      UPDATE candidatos SET vaga_id=vaga_escolhida, vaga=(SELECT titulo FROM vagas WHERE id=vaga_escolhida),
        ...
        revisao_pendente=FALSE,revisao_tipo=NULL,revisao_motivo=NULL,experiencia_declarada=NULL,deslocamento_faixa=NULL,deslocamento_chegada=NULL,
        atendimento_humano_solicitado=FALSE,pendencia_atual='CONFIRMAR_CANDIDATURA',proxima_acao='ESCOLHER_ACAO_VAGA',tentativas_etapa=0,updated_at=NOW()
      WHERE id=c.id RETURNING * INTO c;
  ...
  escolha := CASE WHEN msg ~ '^\s*\d+' THEN (REGEXP_MATCH(msg,'^\s*(\d+)'))[1]::INTEGER ELSE NULL END;

  -- Recuperação simples: explica a etapa em vez de repetir silenciosamente.
  IF n ~ '(JA RESPONDI|NAO ENTENDI|NAO ESTOU ENTENDENDO|MUITO DIFICIL|QUE DIFICIL|COMPLICADO|MEU DEUS)' THEN
    resposta := E'Desculpe pela confusão. Sua candidatura e tudo o que você já informou estão preservados.\n\nAgora falta apenas esta etapa:\n\n' || genesis_chatbot_v1_pergunta_atual(c.etapa::TEXT,c.vaga_id) || E'\n\nPara falar com um recrutador, digite 7.';
    EXIT processamento;
  END IF;

  -- Dúvidas frequentes são respondidas pelos dados reais da vaga, sem mudar a etapa.
  IF c.vaga_id IS NOT NULL AND c.etapa::TEXT NOT IN ('DUVIDAS_VAGA','DUVIDAS_GERAIS','ESCOLHENDO_VAGA','AGUARDANDO_INTENCAO','RECRUTADOR_MENU') THEN
    detalhe := genesis_v13_resposta_duvida_vaga(c.vaga_id,msg);
    IF detalhe IS NOT NULL THEN
      resposta := detalhe || E'\n\nPara continuar de onde paramos:\n\n' || genesis_chatbot_v1_pergunta_atual(c.etapa::TEXT,c.vaga_id);
      EXIT processamento;
    END IF;
  END IF;

  IF c.etapa::TEXT='AGUARDANDO_CTPS' AND n ~ '(NAO TENHO|NUNCA TIVE|SEM CTPS|NAO SEI BAIXAR|NAO CONSIGO GERAR)' THEN
    resposta := E'Sem problemas. Mesmo quem nunca trabalhou com registro possui acesso à CTPS Digital.\n\nAbra o aplicativo CTPS Digital, entre em Contratos, escolha Enviar Carteira de Trabalho e gere o documento completo em PDF.\n\nQuando conseguir, envie o PDF aqui. Para falar com um recrutador, digite 7.';
    EXIT processamento;
  END IF;
...
'||genesis_chatbot_v1_pergunta_atual(nova_etapa,c.vaga_id);
      ...
      ELSE
        ...
        ELSE
          ...
          UPDATE candidatos SET vaga_id=vaga_escolhida,vaga=(SELECT titulo FROM vagas WHERE id=vaga_escolhida),status='EM_PROCESSO',situacao_candidatura='EM_PROCESSO',
            aprovado=NULL,revisao_pendente=FALSE,revisao_tipo=NULL,revisao_motivo=NULL,experiencia_declarada=NULL,deslocamento_faixa=NULL,deslocamento_chegada=NULL,atendimento_humano_solicitado=FALSE,pendencia_atual='CONFIRMAR_CANDIDATURA',proxima_acao='ESCOLHER_ACAO_VAGA',tentativas_etapa=0
          WHERE id=c.id RETURNING * INTO c;
    ...
    WHEN 'AGUARDANDO_NOME' THEN
      IF genesis_v13_nome_valido(msg) THEN
        UPDATE candidatos SET nome=INITCAP(msg) WHERE id=c.id;
        nova_etapa:=genesis_chatbot_v1_etapa_retomada(c.id);
        resposta:='Obrigada.\n\n'||genesis_chatbot_v1_pergunta_atual(nova_etapa,c.vaga_id);
      ELSE resposta:='Não consegui identificar seu nome. Informe somente seu primeiro nome. Exemplo: Maria.'; END IF;
...
BEGIN
  ...
  IF conf IS NOT NULL AND conf>=0.88 AND NULLIF(BTRIM(COALESCE(dados->>'resposta_canonica',dados->>'canonical','')),'') IS NOT NULL THEN canon:=COALESCE(dados->>'resposta_canonica',dados->>'canonical'); END IF;
    ...
    FOR r IN SELECT * FROM genesis_chatbot_v1_processar_texto(p_telefone,COALESCE(canon,p_mensagem),p_mensagem_id,p_session) LOOP
      IF r.etapa='PERGUNTAS_VAGA' THEN
        ...
        IF NULLIF(BTRIM(COALESCE(r.mensagem_whatsapp,'')),'') IS NOT NULL THEN
          UPDATE mensagens m0 SET mensagem=r.mensagem_whatsapp,contexto_snapshot=COALESCE(m0.contexto_snapshot,'{}'::JSONB)||JSONB_BUILD_OBJECT('fluxo','CHATBOT_HIBRIDO_V13_4','etapa','PERGUNTAS_VAGA')
        ...
        END IF;
      END IF;
      IF NULLIF(BTRIM(COALESCE(p_mensagem_id,'')),'') IS NOT NULL THEN UPDATE mensagens SET mensagem=p_mensagem,contexto_snapshot=COALESCE(contexto_snapshot,'{}'::JSONB)||JSONB_BUILD_OBJECT('origem',UPPER(COALESCE(p_origem,'TEXTO')),'entrada_canonica',canon,'fluxo','CHATBOT_HIBRIDO_V13_4') WHERE mensagem_id=p_mensagem_id; END IF;
      candidato_id:=r.candidato_id;telefone:=r.telefone;session:=r.session;mensagem_whatsapp:=r.mensagem_whatsapp;action:=r.action;opcao_numero:=r.opcao_numero;etapa:=r.etapa;status:=r.status;deve_enviar:=r.deve_enviar;RETURN NEXT;
    END LOOP;
  END IF;
  SELECT * INTO c FROM candidatos WHERE candidatos.telefone=REGEXP_REPLACE(COALESCE(p_telefone,''),'\D','','g') LIMIT 1;
