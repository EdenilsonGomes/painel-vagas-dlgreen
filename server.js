...
const candidateStageValues = [
  'PRIMEIRO_CONTATO', 'PERGUNTANDO_IDADE', 'PERGUNTANDO_EXPERIENCIA', 'ESCOLHENDO_VAGA',
...
app.post('/api/vagas/:id/duplicar', async (req, res, next) => {
  ...
  try {
    ...
    const enriched = await client.query(`
      UPDATE vagas SET
        idade_minima = $2,
        entrevista_dias_semana = $3::SMALLINT[],
        entrevista_horarios = $4::JSONB,
        entrevista_duracao_minutos = $5,
        entrevista_busca_dias = $6,
        entrevista_evitar_feriados = $7,
        updated_at = NOW()
      WHERE id = $1 RETURNING *
    `, [result.rows[0].id, Number(v.idade_minima || 25), v.entrevista_dias_semana || [1,2,3,4,5],
      JSON.stringify(v.entrevista_horarios || ['09:00','10:00','14:00','15:00']),
...
app.post('/api/vagas', async (req, res, next) => {
  ...
  try {
    ...
    const enriched = await client.query(`
      ...
      WHERE id = $1
      RETURNING *
    `, [result.rows[0].id, v.idade_minima, v.entrevista_dias_semana,
  ...
  try {
    ...
    const result = await client.query(`
      UPDATE vagas SET
        ...
        aceita_sem_experiencia=$28, exigir_experiencia_compativel=$29,
        cargos_compativeis=$30, cbos_compativeis=$31,
        requisitos_obrigatorios=$32, requisitos_desejaveis=$33,
        quantidade_vagas=$34, formulario_url=$35, publicar_portal=$36,
        destaque_portal=$37, imagem_capa_url=$38, seo_titulo=$39,
        seo_descricao=$40, canal_candidatura=$41, whatsapp_candidatura=$42,
        candidatura_url=$43, candidatura_email=$44, status=$45,
        data_inicio=$46, data_encerramento=$47, updated_at=NOW()
    ...
    const enriched = await client.query(`
      ...
      WHERE id = $1
      RETURNING *
    `, [id, v.idade_minima, v.entrevista_dias_semana,
      JSON.stringify(v.entrevista_horarios), v.entrevista_duracao_minutos,
      v.entrevista_busca_dias, v.entrevista_evitar_feriados]);
    await client.query('COMMIT');
...
app.post('/api/candidatos/:id/ia', async (req, res, next) => {
  const client = await pool.connect();
  try {
    const id = parseId(req.params.id);
    if (!id) return res.status(400).json({ sucesso: false, erro: 'ID de candidato inválido.' });
...
app.post('/api/candidatos/:id/continuar-atendimento', async (req, res, next) => {
  const client = await pool.connect();
  try {
    const id = parseId(req.params.id);
    if (!id) return res.status(400).json({ sucesso: false, erro: 'ID de candidato inválido.' });
...
app.post('/api/candidatos/:id/reprocessar-ctps', async (req, res, next) => {
  try {
    const id = parseId(req.params.id);
    if (!id) return res.status(400).json({ sucesso: false, erro: 'ID de candidato inválido.' });
    if (!CHATBOT_WEBHOOK_URL) {
  ...
  try {
    const response = await fetch(AUDITORIA_IA_WEBHOOK_URL, {
      ...
      body: JSON.stringify({
        ...
        mensagens: messages,
        categorias_permitidas: ['PERGUNTA_IGNORADA','RESPOSTA_FORA_CONTEXTO','INFORMACAO_INVENTADA','CONTRADICAO','NATURALIDADE','INTERPRETACAO_INCORRETA'],
      }),
      signal: controller.signal,
    });
...
async function executeHybridAudit({ start, end, origin, requestedBy }) {
  ...
  try {
    ...
    const candidatesResult = await client.query(`
      SELECT c.*, v.titulo AS vaga_nome, v.idade_minima,
        v.entrevista_horarios, v.entrevista_dias_semana,
        v.entrevista_duracao_minutos, TO_JSONB(v) AS vaga_dados
      FROM candidatos c
      LEFT JOIN vagas v ON v.id = c.vaga_id
      WHERE EXISTS (
    ...
    for (const candidate of candidatesResult.rows) {
      const [messagesResult, documentsResult, interviewsResult, historyResult] = await Promise.all([
        client.query(`SELECT id, quem, mensagem, mensagem_id, created_at FROM mensagens WHERE candidato_id=$1 AND created_at >= $2 AND created_at < $3 ORDER BY created_at ASC, id ASC`, [candidate.id, start, end]),
        client.query(`SELECT id, tipo, titulo, nome_arquivo, status_processamento, classificacao_confianca, resultado, created_at, processado_at FROM documentos WHERE candidato_id=$1 ORDER BY created_at ASC, id ASC`, [candidate.id]),
        client.query(`SELECT id, inicio, fim, status, created_at FROM entrevistas WHERE candidato_id=$1 AND created_at >= $2 AND created_at < $3 ORDER BY created_at ASC`, [candidate.id, start, end]),
      ...
      for (let index = 1; index < iaMessages.length; index += 1) {
        const previous = iaMessages[index - 1];
        const current = iaMessages[index];
        ...
        if (seconds <= 90 && similarity >= 0.90) {
          issues.push({ categoria: 'MENSAGEM_DUPLICADA', gravidade: seconds <= 5 ? 'ALTA' : 'MEDIA',
            titulo: 'Mensagem duplicada ou muito semelhante',
            descricao: `A Evelyn enviou duas respostas com ${Math.round(similarity * 100)}% de similaridade em ${Math.round(seconds)} segundo(s).`,
            ...
            sugestao_correcao: 'Revisar deduplicação, buffer e execuções concorrentes.', mensagem_ia_id: current.id,
      ...
      for (const message of iaMessages) {
        ...
        if (asksForCep(message.mensagem) && /^\d{8}$/.test(String(candidate.cep || '').replace(/\D/g, ''))) {
          issues.push({ categoria: 'PEDIDO_REPETIDO_CEP', gravidade: 'MEDIA', titulo: 'CEP possivelmente solicitado novamente',
            descricao: 'A Evelyn pediu o CEP em uma conversa cujo cadastro já possui um CEP válido.',
            evidencia: { mensagem: message, cep_cadastrado: candidate.cep },
            comportamento_esperado: 'Usar o CEP já salvo e avançar para a pendência seguinte.',
            sugestao_correcao: 'Revisar o contexto enviado à IA e a atualização do CEP no banco.', mensagem_ia_id: message.id,
            anchor: message.id });
        }
        if (mentionsInterviewOffer(message.mensagem)) {
          const age = calculateAgeAt(candidate.data_nascimento, messageDate);
          const minAge = Number(candidate.idade_minima || 25);
          const missing = [];
          if (!/^\d{8}$/.test(String(candidate.cep || '').replace(/\D/g, ''))) missing.push('CEP');
          if (!documents.some((doc) => String(doc.tipo || '').toUpperCase() === 'CTPS' && new Date(doc.created_at) <= messageDate)) missing.push('CTPS');
          if (candidate.aprovado !== true && String(candidate.status || '').toUpperCase() !== 'APROVADO') missing.push('aprovação da triagem');
          if (!String(candidate.apresentacao_profissional || '').trim()) missing.push('apresentação profissional');
          if (age === null) missing.push('data de nascimento');
          else if (age < minAge) missing.push(`idade mínima de ${minAge} anos`);
          if (missing.length) {
      ...
      for (const doc of documents) {
        const pending = ['PENDENTE','PROCESSANDO'].includes(String(doc.status_processamento || '').toUpperCase())
          || String(doc.tipo || '').toUpperCase() === 'PENDENTE';
      ...
      const allowedTransitions = new Map([
        ['PRIMEIRO_CONTATO', new Set(['PRIMEIRO_CONTATO','ESCOLHENDO_VAGA','PERGUNTANDO_IDADE','PERGUNTANDO_EXPERIENCIA'])],
        ['ESCOLHENDO_VAGA', new Set(['ESCOLHENDO_VAGA','PERGUNTANDO_IDADE','PERGUNTANDO_EXPERIENCIA','AGUARDANDO_CTPS_CEP','AGUARDANDO_CTPS','AGUARDANDO_CEP','REPROVADO_PRE_TRIAGEM'])],
        ['PERGUNTANDO_IDADE', new Set(['PERGUNTANDO_IDADE','PERGUNTANDO_EXPERIENCIA','AGUARDANDO_CTPS_CEP','AGUARDANDO_CTPS','AGUARDANDO_CEP','REPROVADO_PRE_TRIAGEM'])],
        ['PERGUNTANDO_EXPERIENCIA', new Set(['PERGUNTANDO_EXPERIENCIA','AGUARDANDO_CTPS_CEP','AGUARDANDO_CTPS','AGUARDANDO_CEP','REPROVADO_PRE_TRIAGEM'])],
        ...
        ['AGUARDANDO_CEP', new Set(['AGUARDANDO_CEP','ANALISANDO_DOCUMENTOS','APROVADO_TRIAGEM','REPROVADO_TRIAGEM'])],
        ['ANALISANDO_DOCUMENTOS', new Set(['ANALISANDO_DOCUMENTOS','AGUARDANDO_CEP','APROVADO_TRIAGEM','REPROVADO_TRIAGEM'])],
        ['APROVADO_TRIAGEM', new Set(['APROVADO_TRIAGEM','AGUARDANDO_APRESENTACAO','GERANDO_OPCOES_ENTREVISTA'])],
      ...
      if (ageNow !== null && ageNow < Number(candidate.idade_minima || 25)
          && !['REPROVADO','ENCERRADO'].includes(String(candidate.status || '').toUpperCase())) {
        issues.push({ categoria: 'IDADE_MINIMA_IGNORADA', gravidade: 'CRITICA', titulo: 'Candidato abaixo da idade mínima avançou',
          descricao: `O candidato possui ${ageNow} anos e a vaga exige no mínimo ${Number(candidate.idade_minima || 25)} anos.`,
          evidencia: { idade: ageNow, idade_minima: candidate.idade_minima, etapa: candidate.etapa, status: candidate.status },
          comportamento_esperado: 'Interromper a pré-triagem e registrar reprovação por idade mínima não atendida.',
          sugestao_correcao: 'Validar a data de nascimento logo após a escolha da vaga.', anchor: `age-${candidate.id}` });
      }
      ...
      try {
        ...
        for (const issue of aiIssues) {
          ...
          if (!['PERGUNTA_IGNORADA','RESPOSTA_FORA_CONTEXTO','INFORMACAO_INVENTADA','CONTRADICAO','NATURALIDADE','INTERPRETACAO_INCORRETA'].includes(category)) continue;
  ...
  try {
    ...
    const [latest, issues, trends, categories] = await Promise.all([
      ...
      pool.query(`SELECT categoria, COUNT(*)::INTEGER AS quantidade
        ...
        GROUP BY categoria ORDER BY quantidade DESC, categoria ASC LIMIT 8`),
    ]);
...
app.use((error, _req, res, _next) => {
  ...
  if (error && ['42703', '42P01'].includes(String(error.code || ''))) {
    return res.status(500).json({
      sucesso: false,
      erro: 'A estrutura do PostgreSQL está incompleta. Execute as migrações anteriores e a migração V6, depois faça o redeploy.',
    });
  }
