    ...
    const [metricas, funil, entrevistas] = await Promise.all([
      pool.query(`
        SELECT
          ...
          (SELECT COUNT(*) FROM entrevistas WHERE status = 'AGENDADA' AND (inicio AT TIME ZONE 'America/Sao_Paulo')::DATE = (NOW() AT TIME ZONE 'America/Sao_Paulo')::DATE)::INTEGER AS entrevistas_hoje,
      ...
      pool.query(`
        ...
        WHERE e.status = 'AGENDADA' AND e.inicio >= NOW()
...
app.post('/api/candidatos/:id/decisao-pos-entrevista', async (req, res, next) => {
  ...
  try {
    ...
    await client.query('COMMIT');
    return res.json({ sucesso: true, mensagem: 'Decisão registrada.', candidato: result.rows[0] });
  } catch (error) {
