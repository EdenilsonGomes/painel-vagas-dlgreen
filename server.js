...
const candidateStageValues = [
  'PRIMEIRO_CONTATO', 'PERGUNTANDO_IDADE', 'PERGUNTANDO_SEXO', 'PERGUNTANDO_EXPERIENCIA', 'ESCOLHENDO_VAGA',
    ...
    for (const candidate of candidatesResult.rows) {
      ...
      for (const message of iaMessages) {
        ...
        if (mentionsInterviewOffer(message.mensagem)) {
          ...
          else if (maxAge !== null && Number.isFinite(maxAge) && age > maxAge) missing.push(`idade máxima de ${maxAge} anos`);
          if (missing.length) {
      ...
      const allowedTransitions = new Map([
        ...
        ['ESCOLHENDO_VAGA', new Set(['ESCOLHENDO_VAGA','PERGUNTANDO_IDADE','PERGUNTANDO_SEXO','PERGUNTANDO_EXPERIENCIA','AGUARDANDO_CTPS_CEP','AGUARDANDO_CTPS','AGUARDANDO_CEP','REPROVADO_PRE_TRIAGEM'])],
        ['PERGUNTANDO_IDADE', new Set(['PERGUNTANDO_IDADE','PERGUNTANDO_SEXO','PERGUNTANDO_EXPERIENCIA','AGUARDANDO_CTPS_CEP','AGUARDANDO_CTPS','AGUARDANDO_CEP','REPROVADO_PRE_TRIAGEM'])],
        ...
        ['ANALISANDO_DOCUMENTOS', new Set(['ANALISANDO_DOCUMENTOS','AGUARDANDO_CEP','PERGUNTANDO_SEXO','APROVADO_TRIAGEM','AGUARDANDO_APRESENTACAO','REPROVADO_TRIAGEM'])],
      ...
      if (rejectedStatus && (/\b(sexo|genero|masculino|feminino)\b/.test(rejectionTextAudit) || rejectionCodeAudit.includes('SEXO'))) {
        issues.push({ categoria: 'REPROVACAO_POR_SEXO', gravidade: 'CRITICA', titulo: 'Possível reprovação automática por sexo',
          descricao: 'O motivo registrado faz referência a sexo ou gênero. A V9.2 trata esse dado apenas como sinalização para revisão humana.',
      ...
      const ageOutsideRange = ageNow !== null && (ageNow < minAgeNow || (maxAgeNow !== null && Number.isFinite(maxAgeNow) && ageNow > maxAgeNow));
      if (ageOutsideRange && !['REPROVADO','ENCERRADO'].includes(String(candidate.status || '').toUpperCase())) {
        ...
        issues.push({ categoria: 'FAIXA_ETARIA_IGNORADA', gravidade: 'CRITICA', titulo: 'Candidato fora da faixa etária avançou',
          ...
          sugestao_correcao: 'Validar a data de nascimento da CTPS, currículo ou informação do candidato logo após a escolha da vaga.', anchor: `age-${candidate.id}` });
      }
  ...
  try {
    ...
    const [issuesResult, feedbackResult, messagesResult, documentsResult, eventsResult, interviewsResult] = candidateIds.length
      ? await Promise.all([
        ...
        pool.query(`SELECT id,candidato_id,quem,mensagem,mensagem_id,created_at FROM mensagens WHERE candidato_id = ANY($1::BIGINT[]) AND created_at >= $2 AND created_at < $3 ORDER BY candidato_id,created_at,id`, [candidateIds, start, end]),
    ...
    const zip = createZipBuffer([
      ...
      { name: 'LEIA-ME.txt', content: 'Pacote anonimizado da Auditoria Genesis IA V9.1. Não contém PDFs, CPF, telefone, CEP, nome civil ou credenciais.\\n' },
    ]);
    res.send(zip);
  } catch (error) { next(error); }
});
