    ...
    const response = await fetch(AUDITORIA_IA_WEBHOOK_URL, {
      ...
      body: JSON.stringify({
        ...
        categorias_permitidas: [
          ...
          'NATURALIDADE','INTERPRETACAO_INCORRETA','NAO_RECONHECEU_DOCUMENTO',
          'RESPOSTA_INCOMPLETA','TOM_INADEQUADO','SOLICITOU_MULTIPLAS_PENDENCIAS',
  ...
  try {
    ...
    for (const candidate of candidatesResult.rows) {
      const [messagesResult, documentsResult, interviewsResult, historyResult] = await Promise.all([
        ...
        client.query(`SELECT id, tipo, titulo, nome_arquivo, mensagem_id, hash_sha256, status_processamento, classificacao_confianca, resultado, created_at, processando_at, processado_at FROM documentos WHERE candidato_id=$1 ORDER BY created_at ASC, id ASC`, [candidate.id]),
      ...
      for (const message of iaMessages) {
        ...
        if (asksForSex(message.mensagem) && (sexKnownBefore || sexAskedBefore)) {
        ...
        if (asksForAge(message.mensagem) && ageKnownBefore) {
        ...
        if (mentionsInterviewOffer(message.mensagem)) {
          ...
          if (!(hasSnapshot ? snap.apresentacao_informada === true : Boolean(String(candidate.apresentacao_profissional || '').trim()))) missing.push('apresentação profissional');
      ...
      if ((candidate.idade_declarada != null || String(candidate.data_nascimento_origem || '').toUpperCase() === 'INFORMADA')
          && candidate.idade_confirmada_documentalmente !== true
        ...
        issues.push({ categoria: 'IDADE_SEM_CONFIRMACAO_DOCUMENTAL', gravidade: 'ALTA', titulo: 'Processo avançou sem confirmação documental da idade',
          ...
          comportamento_esperado: 'Confirmar a idade na CTPS ou currículo antes da entrevista.',
      ...
      if (ageOutsideRange && !['REPROVADO','ENCERRADO'].includes(String(candidate.status || '').toUpperCase())) {
        ...
        issues.push({ categoria: 'FAIXA_ETARIA_IGNORADA', gravidade: 'CRITICA', titulo: 'Candidato fora da faixa etária avançou',
          ...
          sugestao_correcao: 'Validar a data de nascimento confirmada pela CTPS ou currículo antes de liberar a entrevista.', anchor: `age-${candidate.id}` });
      }
