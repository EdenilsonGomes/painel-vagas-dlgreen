...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350" role="img" aria-label="Vaga ${escapeXml(title)}">
  ...
  <g transform="translate(46 62)">
    ...
    <text x="142" y="41" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="27" font-weight="900" fill="#07152e">📣 VAGA ABERTA</text>
  </g>

  <text x="48" y="215" font-family="Arial, Helvetica, sans-serif" font-size="66" font-weight="900" fill="#ffffff">${buildSvgTextLines(titleLines, { x: 48, lineHeight: 69 })}</text>
  <text x="51" y="420" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="800" fill="#fbbf24">INÍCIO IMEDIATO</text>
  <text x="51" y="463" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="700" fill="#dbeafe">${escapeXml(company)}</text>
  ...
  <g transform="translate(38 510)" filter="url(#shadow)">
    ...
    <text x="112" y="41" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="800" fill="#bae6fd">LOCAL</text>
    <text x="112" y="76" font-family="Arial, Helvetica, sans-serif" font-size="25" font-weight="850" fill="#ffffff">${buildSvgTextLines(locationLines, { x: 112, lineHeight: 29 })}</text>
  </g>

  <g transform="translate(38 646)" filter="url(#shadow)">
    <rect width="500" height="116" rx="28" fill="#ffffff" fill-opacity=".11" stroke="#ffffff" stroke-opacity=".65" stroke-width="2"/>
    <circle cx="58" cy="58" r="38" fill="#ffffff" fill-opacity=".16"/><text x="58" y="70" text-anchor="middle" font-size="34">🕐</text>
    <text x="112" y="41" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="800" fill="#bae6fd">ESCALA E HORÁRIO</text>
    <text x="112" y="76" font-family="Arial, Helvetica, sans-serif" font-size="24" font-weight="850" fill="#ffffff">${buildSvgTextLines(scheduleLines, { x: 112, lineHeight: 28 })}</text>
  </g>

  <g transform="translate(38 782)" filter="url(#shadow)">
    <rect width="500" height="128" rx="28" fill="#ffffff" fill-opacity=".96"/>
    <text x="30" y="38" font-family="Arial, Helvetica, sans-serif" font-size="18" font-weight="900" fill="#475569">GANHOS MENSAIS APROXIMADOS</text>
    <text x="30" y="90" font-family="Arial, Helvetica, sans-serif" font-size="43" font-weight="950" fill="#0f172a">${escapeXml(gainsLine)}</text>
    <text x="30" y="116" font-family="Arial, Helvetica, sans-serif" font-size="16" font-weight="700" fill="#64748b">Salário + adicionais e benefícios com valor informado</text>
  </g>
  ...
  <g transform="translate(38 930)" filter="url(#shadow)">
    ...
    <text x="28" y="39" font-family="Arial, Helvetica, sans-serif" font-size="19" font-weight="900" fill="#fbbf24">BENEFÍCIOS E ADICIONAIS</text>
    <text x="30" y="77" font-family="Arial, Helvetica, sans-serif" font-size="23" font-weight="750" fill="#ffffff">${buildSvgTextLines(benefitCardLines, { x: 30, lineHeight: 31, prefix: '• ' })}</text>
  </g>

  <g transform="translate(34 1170)" filter="url(#shadow)">
    <rect width="600" height="132" rx="34" fill="url(#whatsapp)"/>
    <circle cx="68" cy="66" r="45" fill="#ffffff"/><text x="68" y="80" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="39" font-weight="900" fill="#16a34a">☎</text>
    <text x="132" y="49" font-family="Arial, Helvetica, sans-serif" font-size="22" font-weight="800" fill="#ffffff">Candidate-se pelo WhatsApp</text>
    <text x="132" y="94" font-family="Arial, Helvetica, sans-serif" font-size="39" font-weight="950" fill="#ffffff">${escapeXml(contactDisplay)}</text>
  </g>
</svg>`;
...
app.post('/api/vagas/:id/duplicar', async (req, res, next) => {
  ...
  try {
    ...
    const result = await client.query(`
      ...
      SELECT
        empresa_id, $1, titulo || ' — Cópia', cargo, descricao, cidade, estado, bairro,
        endereco_referencia, tipo_contrato, modalidade, escala, horario, salario,
        vale_refeicao_valor, vale_alimentacao_valor, premio_assiduidade_valor,
        outros_beneficios_valor, vale_transporte_descricao, beneficios_observacao,
        possui_insalubridade, percentual_insalubridade, observacao_insalubridade,
        ...
        aceita_sem_experiencia, exigir_experiencia_compativel, cargos_compativeis,
        cbos_compativeis, requisitos_obrigatorios, requisitos_desejaveis,
        quantidade_vagas, formulario_url, 'RASCUNHO', NULL, NULL
      FROM vagas WHERE id = $2
      RETURNING *
    `, [codigo, id]);
    await client.query('COMMIT');
    res.status(201).json({ sucesso: true, mensagem: `Vaga duplicada como ${codigo}.`, vaga: result.rows[0] });
  } catch (error) {
...
app.post('/api/vagas', async (req, res, next) => {
  ...
  try {
    ...
    `, [
      v.empresa_id, codigo, v.titulo, v.cargo, v.descricao, v.cidade, v.estado, v.bairro,
    ...
    await client.query('COMMIT');
    res.status(201).json({ sucesso: true, mensagem: `Vaga ${codigo} cadastrada com sucesso.`, vaga: result.rows[0] });
  } catch (error) {
...
app.put('/api/vagas/:id', async (req, res, next) => {
  ...
  try {
    ...
    const result = await client.query(`
      UPDATE vagas SET
        empresa_id=$1, codigo=$2, titulo=$3, cargo=$4, descricao=$5, cidade=$6,
        estado=$7, bairro=$8, endereco_referencia=$9, tipo_contrato=$10,
        modalidade=$11, escala=$12, horario=$13, salario=$14,
        vale_refeicao_valor=$15, vale_alimentacao_valor=$16,
        premio_assiduidade_valor=$17, outros_beneficios_valor=$18,
        vale_transporte_descricao=$19, beneficios_observacao=$20,
        possui_insalubridade=$21, percentual_insalubridade=$22,
        observacao_insalubridade=$23, beneficios=$24,
        escolaridade_minima=$25, experiencia_minima_meses=$26,
        aceita_sem_experiencia=$27, exigir_experiencia_compativel=$28,
        cargos_compativeis=$29, cbos_compativeis=$30,
        requisitos_obrigatorios=$31, requisitos_desejaveis=$32,
        quantidade_vagas=$33, formulario_url=$34, status=$35,
        data_inicio=$36, data_encerramento=$37, updated_at=NOW()
    ...
    await client.query('COMMIT');
    res.json({ sucesso: true, mensagem: companyChanged ? `Vaga atualizada e recebeu o novo código ${codigo}.` : 'Vaga atualizada com sucesso.', vaga: result.rows[0] });
  } catch (error) {
