...
if (process.env.PANEL_URL && !PANEL_URL_ENV) {
  console.warn(`[CONFIG] PANEL_URL inválido ignorado: ${JSON.stringify(String(process.env.PANEL_URL))}. Use uma URL https:// completa.`);
}
if (ALERTAS_ADMIN_ENABLED && !PANEL_URL) {
  console.warn('[CONFIG] ALERTAS_ADMIN_ENABLED=true, mas PANEL_URL não é válido. Alertas com link ficarão retidos até a configuração ser corrigida.');
}
...
async function resolveVacancyScheduling(client, vacancy, user) {
  ...
  if (recruiterId) {
    ...
    if (!result.rowCount) {
      const error = new Error('O recrutador selecionado não pertence à empresa desta vaga ou está inativo.');
...
app.get('/api/auth/profile', async (req, res, next) => {
  try {
    if (!req.user?.id) {
      return res.status(409).json({ sucesso: false, erro: 'Este acesso ainda usa o login legado. Migre o usuário para uma conta individual antes de editar o perfil.' });
    }
    const result = await pool.query(`
      ...
      LIMIT 1
    `, [req.user.id]);
    if (!result.rowCount) return res.status(404).json({ sucesso: false, erro: 'Usuário não encontrado ou inativo.' });
...
app.put('/api/auth/profile', async (req, res, next) => {
  try {
    if (!req.user?.id) {
      return res.status(409).json({ sucesso: false, erro: 'Este acesso ainda usa o login legado e não pode ser editado por esta tela.' });
    }
    ...
    if (duplicate.rowCount) return res.status(409).json({ sucesso: false, erro: 'Esse usuário de acesso já está em uso.' });
    ...
    if (!baseUpdate.rowCount) return res.status(404).json({ sucesso: false, erro: 'Usuário não encontrado ou inativo.' });

    // Campos de alertas vieram na V15. O fallback mantém a edição básica funcional
    // caso uma instalação legada ainda não tenha essas colunas.
    try {
      await pool.query(`
        UPDATE app_usuarios
      ...
      console.warn('[PERFIL V18] Colunas opcionais de alerta ainda não existem; nome e usuário foram atualizados.');
    }
    ...
    if (String(error.code || '') === '23505') return res.status(409).json({ sucesso: false, erro: 'Esse usuário de acesso já está em uso.' });
    return next(error);
  }
});
...
app.post('/api/auth/change-password', async (req, res, next) => {
  try {
    if (!req.user?.id) {
      return res.status(409).json({ sucesso: false, erro: 'Este acesso ainda usa o login legado e não pode alterar a senha por esta tela.' });
    }
    ...
    if (!current.rowCount) return res.status(404).json({ sucesso: false, erro: 'Usuário não encontrado ou inativo.' });
    const valid = await verifyPassword(senhaAtual, current.rows[0].senha_hash);
    if (!valid) return res.status(401).json({ sucesso: false, erro: 'A senha atual está incorreta.' });
    ...
    const timelineResult = isAdminUser ? await pool.query(`
      ...
      FROM (
        ...
        SELECT
          ...
          CONCAT(
            'De: ', COALESCE(NULLIF(valor_anterior, ''), 'não informado'),
            ' → Para: ', COALESCE(NULLIF(valor_novo, ''), 'não informado'),
        ...
        SELECT
          'AUDITORIA'::TEXT AS tipo,
          CASE
            WHEN modo = 'CORRIGIR_E_CONTINUAR' THEN 'Correção técnica e continuidade'
            ELSE 'Correção técnica'
          END AS titulo,
          CONCAT(
            COALESCE(status_anterior, '—'), ' · ', COALESCE(etapa_anterior, '—'),
            ' → ', COALESCE(status_novo, '—'), ' · ', COALESCE(etapa_nova, '—'),
            CASE WHEN motivo IS NOT NULL AND BTRIM(motivo) <> '' THEN ' | Motivo: ' || motivo ELSE '' END,
            CASE WHEN alterado_por_nome IS NOT NULL AND BTRIM(alterado_por_nome) <> '' THEN ' | Por: ' || alterado_por_nome ELSE '' END
          ) AS descricao,
          created_at
...
app.get('/api/documentos/:id/visualizar', async (req, res, next) => {
  try {
    ...
    const documento = result.rows[0];
    if (!documento.conteudo) {
      return res.status(404).json({ sucesso: false, erro: 'Visualização indisponível para este documento antigo.' });
    }
...
app.post('/api/revisoes/lote/decidir', async (req, res, next) => {
  ...
  try {
    ...
    if (!entries.length || entries.length > 100) return res.status(400).json({ sucesso:false, erro:'Selecione entre 1 e 100 revisões.' });
    ...
    if (decisions.some((entry) => !entry.id || !['APROVAR','NAO_APROVAR'].includes(entry.decisao))) {
      return res.status(400).json({ sucesso:false, erro:'Há uma decisão inválida no lote.' });
    }
    await client.query('BEGIN');
    const results = [];
    for (const entry of decisions) {
      ...
      if (!result.rowCount) {
        const error = new Error(`A revisão #${entry.id} não está mais pendente.`);
    ...
    res.json({ sucesso:true, mensagem:`${results.length} decisões registradas.`, resultados:results, acionamentos });
  } catch (error) {
    try { await client.query('ROLLBACK'); } catch {}
    next(error);
  } finally { client.release(); }
...
app.post('/api/admin/candidatos/:id/ctps/decisao-manual', requireAdmin, async (req, res, next) => {
  ...
  try {
    ...
    const enviarMensagem = req.body?.enviar_mensagem === true;
    if (!id) return res.status(400).json({ sucesso:false, erro:'ID de candidato inválido.' });
    if (!['APROVAR','REPROVAR'].includes(decisao)) return res.status(400).json({ sucesso:false, erro:'Selecione uma decisão manual válida.' });
    ...
    if (String(candidate.etapa || '').toUpperCase() === 'REPROVADO_POS_ENTREVISTA') {
      await client.query('ROLLBACK');
      return res.status(409).json({ sucesso:false, erro:'A decisão manual da CTPS não pode substituir uma decisão registrada após entrevista. Use a área “Resultado após entrevista” para esse caso.' });
    }
    if (['CONTRATADO','ENCERRADO'].includes(String(candidate.status || '').toUpperCase()) && decisao === 'REPROVAR') {
      await client.query('ROLLBACK');
      return res.status(409).json({ sucesso:false, erro:'O processo já está encerrado. Reabra a candidatura antes de alterar a decisão da CTPS.' });
    }
    ...
    if (decisao === 'REPROVAR') {
      ...
      if (!allowedReasons.includes(reasonCode)) {
        await client.query('ROLLBACK');
        return res.status(400).json({ sucesso:false, erro:'Selecione o motivo da decisão da CTPS.' });
      }
      if (reasonCode === 'OUTRO' && !observacao) {
        await client.query('ROLLBACK');
        return res.status(400).json({ sucesso:false, erro:'Descreva o motivo da reprovação manual.' });
      }
      ...
      await client.query(`
        UPDATE candidatos SET
          ...
          ia_pausa_motivo='Decisão manual da CTPS',updated_at=NOW()
        WHERE id=$1
      ...
      mensagemCandidato = `${candidateFirstName(candidate) ? `${candidateFirstName(candidate)}, ` : ''}após a revisão da sua documentação, neste momento não será possível continuar na vaga ${candidate.vaga_nome}. Seu cadastro poderá ser considerado em futuras oportunidades compatíveis.`;
      ...
      await client.query(`INSERT INTO eventos(candidato_id,evento,descricao,created_at) VALUES($1,'CTPS_DECISAO_MANUAL_REPROVADA',$2,NOW())`,[
        id,`Decisão manual por ${userName}. Motivo: ${reasonInfo.label}.${observacao ? ` Observação: ${observacao}` : ''} Candidato avisado: ${enviarMensagem ? 'SIM' : 'NÃO'}.`,
      ]);
    } else {
      ...
      await client.query(`
        UPDATE candidato_revisoes SET status='APROVADO',decisao='APROVAR',decisao_motivo=$2,
          decidido_por=$3,decidido_em=NOW(),updated_at=NOW()
        WHERE candidato_id=$1 AND status='PENDENTE' AND tipo IN ('EXCECAO_EXPERIENCIA','REVISAO_DOCUMENTAL')
      `,[id,observacao || 'Aprovação manual da CTPS',userName]).catch(()=>{});
      mensagemCandidato = alreadyApproved
        ? `${candidateFirstName(candidate) ? `${candidateFirstName(candidate)}, ` : ''}sua análise documental foi revisada e sua aprovação nesta etapa foi confirmada. Seu processo continua normalmente.`
        : `${candidateFirstName(candidate) ? `${candidateFirstName(candidate)}, ` : ''}sua análise documental foi revisada e você está apto(a) para continuar na vaga ${candidate.vaga_nome}. Responda por aqui para seguirmos com as próximas etapas.`;
      resumoPainel = alreadyApproved ? 'A aprovação da CTPS foi confirmada manualmente.' : 'CTPS aprovada manualmente.';
      await client.query(`INSERT INTO eventos(candidato_id,evento,descricao,created_at) VALUES($1,'CTPS_DECISAO_MANUAL_APROVADA',$2,NOW())`,[
        id,`Decisão manual por ${userName}.${observacao ? ` Observação: ${observacao}` : ''} Candidato avisado: ${enviarMensagem ? 'SIM' : 'NÃO'}.`,
      ]);
    }
...
app.get('/api/admin/documentos/auditoria-ctps.zip', requireAdmin, async (req, res, next) => {
  try {
    ...
    if (!['TODOS', 'APROVADOS', 'REPROVADOS'].includes(resultado)) {
      return res.status(400).json({ sucesso: false, erro: 'Filtro de resultado inválido.' });
    }
    ...
    for (const row of selected) {
      ...
      if (!row.arquivo_armazenado || !documentId) {
        skipReason.set(documentId || `cand-${row.candidato_id}`, 'PDF bruto não está armazenado no banco.');
        continue;
      }
      if (size <= 0) {
        skipReason.set(documentId, 'Tamanho do PDF não pôde ser determinado.');
        continue;
      }
      if (plannedBytes + size > MAX_EXPORT_BYTES) {
        skipReason.set(documentId, 'PDF não incluído porque o pacote atingiu o limite de 120 MB.');
        continue;
      }
      plannedBytes += size;
    ...
    const csvRows = selected.map((row) => {
      ...
      return {
        ...
        pdf_incluido: Boolean(content) ? 'SIM' : 'NÃO',
        observacao_pdf: content ? '' : (skipReason.get(documentId) || 'PDF bruto indisponível.'),
      };
    });
    ...
    const summary = {
      ...
      observacao: 'A classificação APROVADO/REPROVADO usa o campo candidatos.aprovado, preservando a decisão de triagem mesmo quando o status atual mudou depois da entrevista.',
    };
    ...
    entries.unshift(
      ...
      { name: 'auditoria_ctps.csv', content: rowsToCsv(csvRows, [
        ...
        ['vaga', 'Vaga'],
        ['experiencia_minima_meses', 'Experiência mínima da vaga (meses)'],
        ['maior_experiencia_compativel_dias', 'Maior experiência compatível (dias)'],
        ['maior_experiencia_compativel_texto', 'Maior experiência compatível'],
        ['cargo_vinculo_utilizado', 'Cargo do vínculo utilizado'],
        ['cbo_vinculo_utilizado', 'CBO do vínculo utilizado'],
        ['periodo_vinculo_utilizado', 'Período do vínculo utilizado'],
        ['experiencias_extraidas', 'Experiências extraídas da CTPS'],
        ['motivo_reprovacao_codigo', 'Código da reprovação'],
        ['motivo_reprovacao_categoria', 'Categoria da reprovação'],
        ['motivo_reprovacao_detalhe', 'Detalhe da reprovação'],
        ...
        ['classificacao_confianca', 'Confiança classificação'],
        ['pdf_incluido', 'PDF incluído no ZIP'],
        ['observacao_pdf', 'Observação do PDF'],
      ]) },
      {
        name: 'LEIA-ME.txt',
        content: [
          'GENESIS IA — Exportação de auditoria de CTPS',
          '',
          'Este pacote contém documentos pessoais reais e deve ser tratado como material restrito de RH.',
          'A pasta aprovados/ reúne candidatos cujo campo aprovado=true após a triagem documental.',
          'A pasta reprovados/ reúne candidatos cujo campo aprovado=false após a triagem documental.',
          'O arquivo auditoria_ctps.csv cruza a decisão histórica com os dados de experiência já armazenados.',
          'Documentos antigos sem PDF bruto continuam no CSV com PDF incluído = NÃO.',
          'O exportador limita o conteúdo bruto a 120 MB para proteger a estabilidade do painel.',
          '',
        ].join('\n'),
      },
    );
...
app.use((error, req, res, _next) => {
  ...
  if (error?.name === 'WahaRequestError') {
    return res.status(Number(error.status || 502)).json({
      sucesso: false,
      erro: error.message || 'Não foi possível concluir a operação no WAHA.',
  ...
  if (Number.isInteger(Number(error?.statusCode)) && Number(error.statusCode) >= 400 && Number(error.statusCode) < 600) {
    return res.status(Number(error.statusCode)).json({
      sucesso: false,
      erro: error.message || 'Não foi possível concluir a operação.',
