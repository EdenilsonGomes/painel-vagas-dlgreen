...
function compactJoined(parts, fallback = 'A combinar', separator = ' · ') {
  const text = (parts || []).filter(Boolean).map((part) => String(part).trim()).filter(Boolean).join(separator);
  return text || fallback;
}
...
function buildPromotionAssets(vacancy) {
  ...
  const neighborhood = compactJoined([
    vacancy.bairro,
    compactJoined([vacancy.cidade, vacancy.estado], '', ' - '),
  ], 'Local a combinar', ' · ');
  const salary = formatMoneyBRL(vacancy.salario);
  const schedule = compactJoined([
    vacancy.escala ? `Escala ${vacancy.escala}` : '',
    vacancy.horario ? `Horário ${vacancy.horario}` : '',
  ], 'Horário a combinar');
  const benefitsLines = listLines(vacancy.beneficios, ['Benefícios informados no processo seletivo']).slice(0, 5);
  const benefitsInline = benefitsLines.join(' | ');
  ...
  const titleLines = wrapSvgText(title.toUpperCase(), 22, 3);
  const locationLines = wrapSvgText(neighborhood, 26, 3);
  const scheduleLines = wrapSvgText(schedule, 28, 3);
  const salaryLines = wrapSvgText(salary, 22, 2);
  const benefitText = benefitsLines.map((line) => `• ${line}`);
  const benefitTspans = benefitText.map((line, index) => `<tspan x="94" dy="${index === 0 ? 0 : 34}">${escapeXml(line)}</tspan>`).join('');
  const titleTspans = titleLines.map((line, index) => `<tspan x="78" dy="${index === 0 ? 0 : 58}">${escapeXml(line)}</tspan>`).join('');
  const locationTspans = locationLines.map((line, index) => `<tspan x="110" dy="${index === 0 ? 0 : 34}">${escapeXml(line)}</tspan>`).join('');
  const scheduleTspans = scheduleLines.map((line, index) => `<tspan x="596" dy="${index === 0 ? 0 : 34}">${escapeXml(line)}</tspan>`).join('');
  const salaryTspans = salaryLines.map((line, index) => `<tspan x="110" dy="${index === 0 ? 0 : 42}">${escapeXml(line)}</tspan>`).join('');
...
<svg xmlns="http://www.w3.org/2000/svg" width="1080" height="1350" viewBox="0 0 1080 1350" role="img" aria-label="Divulgação da vaga ${escapeXml(title)}">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="#07254a"/>
      <stop offset="35%" stop-color="#0a3d79"/>
    ...
    <filter id="shadow"><feDropShadow dx="0" dy="16" stdDeviation="24" flood-color="#020617" flood-opacity=".23"/></filter>
  </defs>
  <rect width="1080" height="1350" rx="44" fill="url(#bg)"/>
  <rect x="0" y="0" width="430" height="1350" fill="rgba(5, 23, 52, .38)"/>
  <circle cx="915" cy="235" r="210" fill="#ffffff" opacity=".08"/>
  <circle cx="110" cy="1195" r="220" fill="#ffffff" opacity=".06"/>

  <rect x="34" y="42" width="278" height="62" rx="30" fill="#facc15"/>
  <text x="173" y="83" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="900" fill="#082f49">VAGA ABERTA</text>
  <text x="30" y="150" font-family="Arial, Helvetica, sans-serif" font-size="22" font-weight="800" fill="#f8fafc" opacity=".88">${escapeXml(theme.label)}</text>
  <text x="78" y="228" font-family="Arial, Helvetica, sans-serif" font-size="60" font-weight="900" fill="#ffffff">${titleTspans}</text>
  <text x="80" y="390" font-family="Arial, Helvetica, sans-serif" font-size="28" font-weight="800" fill="#facc15">| Início imediato</text>

  <g filter="url(#shadow)" transform="translate(0, 40)">${theme.icon}</g>
  <rect x="520" y="116" width="490" height="560" rx="34" fill="#ffffff" opacity=".13"/>
  <text x="765" y="718" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="20" font-weight="700" fill="#e2e8f0">Arte automática de divulgação da vaga</text>

  <rect x="36" y="472" width="470" height="156" rx="28" fill="#0b3b70" stroke="#cbd5e1" stroke-opacity=".8"/>
  <circle cx="86" cy="548" r="46" fill="#1567ad" stroke="#7dd3fc" stroke-width="4"/>
  <text x="86" y="563" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="900" fill="#ffffff">📍</text>
  <text x="146" y="530" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="800" fill="#dbeafe">BAIRRO / LOCAL</text>
  <text x="110" y="575" font-family="Arial, Helvetica, sans-serif" font-size="31" font-weight="800" fill="#ffffff">${locationTspans}</text>

  <rect x="542" y="472" width="502" height="156" rx="28" fill="#0b3b70" stroke="#cbd5e1" stroke-opacity=".8"/>
  <circle cx="596" cy="548" r="46" fill="#1567ad" stroke="#7dd3fc" stroke-width="4"/>
  <text x="596" y="562" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="31" font-weight="900" fill="#ffffff">🕐</text>
  <text x="654" y="530" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="800" fill="#dbeafe">HORÁRIO DE TRABALHO</text>
  <text x="596" y="575" font-family="Arial, Helvetica, sans-serif" font-size="29" font-weight="800" fill="#ffffff">${scheduleTspans}</text>

  <rect x="36" y="652" width="470" height="148" rx="28" fill="#0b3b70" stroke="#cbd5e1" stroke-opacity=".8"/>
  <circle cx="86" cy="726" r="46" fill="#1567ad" stroke="#7dd3fc" stroke-width="4"/>
  <text x="86" y="742" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="900" fill="#ffffff">💰</text>
  <text x="146" y="708" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="800" fill="#dbeafe">SALÁRIO</text>
  <text x="110" y="756" font-family="Arial, Helvetica, sans-serif" font-size="38" font-weight="900" fill="#facc15">${salaryTspans}</text>

  <rect x="36" y="826" width="1008" height="248" rx="28" fill="#0b3b70" stroke="#cbd5e1" stroke-opacity=".8"/>
  <circle cx="86" cy="900" r="46" fill="#1567ad" stroke="#7dd3fc" stroke-width="4"/>
  <text x="86" y="916" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="34" font-weight="900" fill="#ffffff">🎁</text>
  <text x="146" y="883" font-family="Arial, Helvetica, sans-serif" font-size="21" font-weight="800" fill="#dbeafe">BENEFÍCIOS</text>
  <text x="94" y="935" font-family="Arial, Helvetica, sans-serif" font-size="30" font-weight="700" fill="#ffffff">${benefitTspans}</text>

  <rect x="36" y="1103" width="1008" height="178" rx="34" fill="url(#cta)"/>
  <text x="540" y="1164" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="25" font-weight="800" fill="#ffffff">ENVIAR MENSAGEM NO WHATSAPP PARA MAIS INFORMAÇÕES</text>
  <text x="540" y="1234" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-size="54" font-weight="900" fill="#ffffff">${escapeXml(contactDisplay)}</text>
</svg>`;
