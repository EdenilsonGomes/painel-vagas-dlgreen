...
(async()=>{try{const sql=fs.readFileSync(path.join(__dirname,'..','sql','27_GENESIS_IA_V16_ATENDIMENTOS_HANDOFF_DOCUMENTOS.sql'),'utf8');await pool.query(sql);console.log('Migração Genesis IA V16 aplicada com sucesso.');}catch(e){console.error(e);process.exitCode=1;}finally{await pool.end();}})();
