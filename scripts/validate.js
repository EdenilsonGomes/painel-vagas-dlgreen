...
[
  ...
  'lib/screening-v13.js', 'lib/demos-v13.js', 'lib/operations-v14.js',
  'public/app.js', 'public/admin.js', 'public/login.js', 'public/portal-publicacoes.js',
  'public/theme-init.js', 'public/screening-v13.js', 'public/demos-v13.js', 'public/demo-client.js', 'public/operations-v14.js',
...
for (const token of [
  'themeToggleButton', 'vacancyForm', 'candidateConversation', 'modern-v13.css', 'modern-v14.css',
  'view-demos', 'demoCreateForm', 'screeningQuestionsBuilder', 'candidateScreeningSection',
  'view-brands', 'brandEditorDialog', 'vacancyWizardSteps', 'agendaSettingsDialog',
  'prospectFilterGrid', 'prospectingConnectionDialog', 'outreachDialog',
...
checkCss('public/modern-v14.css', ['vacancy-wizard-steps', 'brand-company-card', 'prospect-filter-grid', 'review-batch-toolbar', '@media']);
checkCss('public/demo.css', ['demo-activation-grid', 'demo-qr-frame', '@media']);

const server = read('server.js');
for (const token of [
  'registerDemosV13', 'registerScreeningV13', 'registerOperationsV14', 'DEMO_CHATBOT_WEBHOOK_URL',
  'publicar_portal', 'PORTAL_IMAGE_ORIGIN', 'GRUPOS_STATUS_LEGADO', 'recrutador_responsavel_id',
...
const migrationV14 = read('sql/19_GENESIS_RECRUITING_OS_V14_MARCA_AGENDA_PROSPECCAO.sql');
for (const token of [
  'empresa_marcas', 'recrutador_agendas', 'vaga_artes_ia', 'INCOMPATIBILIDADE_SEXO',
  'prospeccao_envios', 'prospeccao_respostas', 'contato_autorizado',
]) assert(migrationV14.includes(token), `Migração V14 incompleta: ${token}.`);

const operationsV14 = read('lib/operations-v14.js');
for (const token of [
  '/v1/images/generations', 'gpt-image-2', 'classifyInboundReply', "'/api/sendText'",
  'confirmar_autorizacao', 'PROSPECTING_OUTREACH_ENABLED',
]) assert(operationsV14.includes(token), `Operações V14 sem proteção esperada: ${token}.`);

const interviewWorkflow = JSON.parse(read('n8n/Genesis-IA-Entrevistas-Google-Meet-Estatico-V1-Credenciais-Preservadas.json'));
const interviewSerialized = JSON.stringify(interviewWorkflow);
for (const token of ['recrutador_agendas', 'a.google_calendar_id', 'a.whatsapp_alerta']) {
  assert(interviewSerialized.includes(token), `Workflow de entrevistas não herda ${token}.`);
}

const envExample = read('.env.example');
for (const token of ['WAHA_BASE_URL=', 'WAHA_API_KEY=', 'DEMO_CHATBOT_WEBHOOK_URL=', 'DEMO_TRIAL_DAYS=7', 'OPENAI_API_KEY=', 'PROSPECTING_WAHA_SESSION=', 'PROSPECTING_OUTREACH_ENABLED=false']) {
  assert(envExample.includes(token), `.env.example sem ${token}.`);
}

const packageJson = JSON.parse(read('package.json'));
assert(packageJson.version === '14.0.0', 'A versão do pacote deve ser 14.0.0.');
assert(packageJson.scripts['migrate:panel'] && packageJson.scripts.preflight, 'Scripts de deploy V14 ausentes.');
...
console.log('Validação V14 concluída: painel white-label, agenda, artes IA, prospecção assistida, triagem, PDF, áudio e workflows estão consistentes.');
