...
assert(html.includes('?v=1510'), 'Os assets V15.1 devem usar cache version 1510.');
...
for (const token of ['registerDemosV13', 'registerScreeningV13', 'registerOperationsV14', 'registerDivulgacaoV1', 'registerAtendimentoV15']) {
  assert(server.includes(token), `Backend sem ${token}.`);
}

const packageJson = JSON.parse(read('package.json'));
assert(packageJson.version === '15.1.0', 'A versão do pacote deve ser 15.1.0.');
assert(packageJson.scripts['migrate:panel'] && packageJson.scripts.preflight, 'Scripts de implantação ausentes.');

console.log('Validação V15.1 concluída: Chat UI/documentos de recrutador, Central de Divulgação e atendimento humano estão consistentes.');
