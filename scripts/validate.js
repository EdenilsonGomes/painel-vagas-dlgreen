...
assert(html.includes('?v=1410'), 'Os assets V14.1 devem usar cache version 1410.');
...
for (const token of ['registerDemosV13', 'registerScreeningV13', 'registerOperationsV14']) {
  assert(server.includes(token), `Backend sem ${token}.`);
}

const packageJson = JSON.parse(read('package.json'));
assert(packageJson.version === '14.1.0', 'A versão do pacote deve ser 14.1.0.');
assert(packageJson.scripts['migrate:panel'] && packageJson.scripts.preflight, 'Scripts de implantação ausentes.');

console.log('Validação V14.1 concluída: painel compacto, agenda, filtros por coluna e patch V13.4 estão consistentes.');
