...
async function main() {
  ...
  try {
    ...
    const v14Columns = await pool.query(`
      SELECT table_name,column_name FROM information_schema.columns
      WHERE table_schema='public' AND (
        (table_name='app_usuarios' AND column_name='empresa_id') OR
        (table_name='vagas' AND column_name IN ('recrutador_responsavel_id','agenda_personalizada')) OR
        (table_name='prospeccao_leads' AND column_name IN ('contato_autorizado','resposta_tipo'))
      )
    `);
    const foundV14 = new Set(v14Columns.rows.map((row) => `${row.table_name}.${row.column_name}`));
    const requiredV14 = ['app_usuarios.empresa_id','vagas.recrutador_responsavel_id','vagas.agenda_personalizada','prospeccao_leads.contato_autorizado','prospeccao_leads.resposta_tipo'];
    const missingV14Columns = requiredV14.filter((name) => !foundV14.has(name));
    if (missingV14Columns.length) throw new Error(`Colunas V14 ausentes: ${missingV14Columns.join(', ')}. Execute npm run migrate:panel.`);
    ...
    console.log('Pré-checagem do painel concluída.', {
      ...
      demosSeteDias: 'ok',
      marcaEmpresas: 'ok',
      agendasIndividuais: 'ok',
      artesIA: process.env.OPENAI_API_KEY ? 'configurada' : 'aguardando OPENAI_API_KEY',
      prospeccaoAssistida: String(process.env.PROSPECTING_OUTREACH_ENABLED || 'false').toLowerCase() === 'true' ? 'habilitada' : 'modo seguro/desabilitada',
