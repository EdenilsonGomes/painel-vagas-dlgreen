...
const performance = read('lib/dashboard-performance.js');
...
ok(app.includes("dashboardPeriod: '1D'") && app.includes('drawDashboardPerformanceChart'), 'Comportamento analítico da Visão geral ausente.');
ok(server.includes('vagas_atencao:') && server.includes('loadDashboardPerformance(pool, period.days)') && performance.includes('candidaturas_periodo_anterior'), 'API da Visão geral não entrega análises e saúde das vagas.');
ok(performance.includes('::INTEGER AS primeira_analise_minutos'), 'Conversão da mediana de primeira análise ausente.');
ok(!/\)::INTEGER,\s*0\)\s+AS primeira_analise_minutos/.test(performance), 'COALESCE da primeira análise contém cast/argumento em posição inválida.');
