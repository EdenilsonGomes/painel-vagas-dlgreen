...
async function main() {
  ...
  console.log(`Diagnóstico Geo V1 em ${label}`);
  try {
    ...
    const vagas = await pool.query(`
      SELECT gv.vaga_id,gv.cep,g.status,g.cidade,g.bairro,g.latitude,g.longitude
    ...
    const amostra = await pool.query(`
      SELECT c.id AS candidato_id,c.nome,
        ...
        gc.status AS candidato_geo_status,gc.latitude AS candidato_lat,gc.longitude AS candidato_lon,
        vg.status AS vaga_geo_status,vg.latitude AS vaga_lat,vg.longitude AS vaga_lon,
...
main().catch((error) => { console.error('Diagnóstico Geo V1 falhou:', error); process.exitCode = 1; });
